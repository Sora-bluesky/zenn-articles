...
第8回でmorning-news Skillを作ったとき、毎朝のニュース要約に一文が混じっていた。「X検索は利用不可だったため、HN/Web中心で代替しました」。エージェントは正直に、使えなかった道具を申告していた。

検索の質は、Skillの出力をそのまま左右する。第8回までのHermes Agentは「決まった時刻に、覚えた手順で動く」ところまで来たが、その手順が見にいく情報源がまだ整っていない。第9回は、Hermes Agentが使える複数の検索バックエンドを整理し、**Web検索**と**X検索**を別の道具として有効化する。
...
ポイントは、3つを別々に持てること。Web検索は無制限のSearXNGに任せ、有料枠のあるFirecrawlは「素のページでは歯が立たない難しいサイト」だけに温存できる。
...
![cat config.yamlのweb:セクション。backend: ddgsで、search_backend/extract_backendが空の初期状態(ホスト名/IPはマスク)](/images/hermes-vps/hermes-vps-09-config-web-before.png)
...
このシリーズでは**SearXNG**(検索)と**Firecrawl**(本文取得)を組み合わせる。理由はシンプルで、検索は回数を気にせず使いたいから無制限のSearXNG、本文取得は質の高いFirecrawlを「ここぞ」という難しいページだけに使う、という役割分担にすると無料枠を長く保てる。
...
![mkdirで~/searxngを作り、nanoでdocker-compose.ymlを開いた直後の画面(ホスト名/IPはマスク)](/images/hermes-vps/hermes-vps-09-searxng-compose-edit.png)
...
![保存後のdocker-compose.ymlの中身。127.0.0.1:8888へのポート割り当てが見える画面](/images/hermes-vps/hermes-vps-09-searxng-compose-saved.png)
...
![保存後のsettings.ymlの中身。formatsにhtmlとjsonが並んでいる画面](/images/hermes-vps/hermes-vps-09-searxng-settings.png)
...
![docker compose up -d実行直後。イメージのpullとコンテナ起動のログが流れている画面](/images/hermes-vps/hermes-vps-09-searxng-up.png)

![docker compose psでsearxngがUp状態になっている画面](/images/hermes-vps/hermes-vps-09-searxng-ps.png)

最後の`curl`でJSONが返ってくれば、検索エンジンとして動いている。これがHermes Agentから叩く先になる。

![curlでlocalhost:8888にJSON形式の検索リクエストを送り、JSON応答が返ってきた画面](/images/hermes-vps/hermes-vps-09-searxng-curl-json.png)
...
![Firecrawlのサインアップ画面](/images/hermes-vps/hermes-vps-09-firecrawl-signup.png)

![Firecrawlダッシュボードのトップ。無料枠1,000/月の表示が見える画面](/images/hermes-vps/hermes-vps-09-firecrawl-dashboard.png)

ダッシュボードでAPIキー(`fc-`で始まる文字列)を発行する。

![APIキー発行画面。fc-で始まるキーが表示された画面(キー本体はマスク)](/images/hermes-vps/hermes-vps-09-firecrawl-apikey.png)
...
![1Passwordの保管庫Hermes-Prodに新規アイテム「Firecrawl」を作りAPIキーを保存した画面(キー本体はマスク)](/images/hermes-vps/hermes-vps-09-firecrawl-1password.png)
...
![secrets.envにFIRECRAWL_API_KEY=op://...とSEARXNG_URL=...を追記した画面](/images/hermes-vps/hermes-vps-09-secrets-env.png)
...
![config.yamlのweb:セクションにsearch_backend: searxngとextract_backend: firecrawlが入った画面](/images/hermes-vps/hermes-vps-09-config-backend.png)
...
![systemctl --user is-active hermes-gatewayがactiveと返った画面(再起動成功の確認)](/images/hermes-vps/hermes-vps-09-gateway-restart.png)
...
![Telegramで検索を依頼し、3件以上のURLと一行要約が返ってきた画面(自分の表示名/ハンドル/chat IDはマスク)](/images/hermes-vps/hermes-vps-09-telegram-search.png)
...
![docker compose logsにSearXNGがクエリを処理したログが出ている画面。gatewayからsearxngが使われた実証](/images/hermes-vps/hermes-vps-09-searxng-log.png)
...
![Telegramで本文取得を依頼し、ページの見出し一覧が返ってきた画面。今回はcurl経由で手段はエージェント判断](/images/hermes-vps/hermes-vps-09-telegram-extract.png)
...
![hermes doctorの出力に✓ xAI OAuth (logged in)と✓ x_searchが並び、config.yamlのx_searchセクションも見える画面(ホスト名/IPはマスク)](/images/hermes-vps/hermes-vps-09-doctor-xsearch.png)
...
![x_searchでXの議論・反応の要約と投稿URLが返ってきた画面。いいね/RT等の数値は出ていない(自分のハンドル/bot名/chat IDはマスク)](/images/hermes-vps/hermes-vps-09-xsearch-result.png)
...
- 8つの検索バックエンドから、無制限の**SearXNG**(検索)と**Firecrawl**(難しいページの本文取得)を選んで組み合わせた
