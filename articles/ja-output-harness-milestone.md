...
- 違反を検出したら、Codex が同じターン内で自己修正する。追加で走るのは違反が出たときだけで、実測 +15% 程度（既定の `strict-lite` モード）
...
加えて、`finalize` のレスポンス（違反 JSON）を次ターンで読むための input トークンも増える。毎日使うと、月末の請求書で痛みが可視化される。
...
違反時に continuation が走るぶんだけ追加トークンが発生するが、それも実測 +15% 程度。違反ゼロなら 0%。「下書きを毎回 2 回払う」旧構造に比べて、ならすと大幅に安い。

## 実測 ok 率 23.8% の見方

n=21 の dogfood で、初手で `ok: true` を返す率は 23.8%、Wilson 95% CI [10.6%, 45.1%] だった。一見、通す率が低い。
...
n=21 と小さいサンプルだが、Wilson 区間を載せて誠実に見せている。前作で出した「32→0」は特定ベンチ上の数字で、汎用運用でも同じになるとは限らなかった。今回は日常の dogfood から取った数字を、信頼区間つきで置いた。
