...
emoji: "☁️"
type: "idea"
topics: ["GoogleCloud", "GCP", "初心者", "無料枠", "GeminiAPI"]
published: false
---

## 目次

1. [Google Cloud Console とは](#google-cloud-console-とは)
2. [無料トライアル・有料アカウント・無料枠の違い](#無料トライアル・有料アカウント・無料枠の違い)
3. [課金しないとできないこと](#課金しないとできないこと)
4. [課金しなくても使えるサービス](#課金しなくても使えるサービス)
5. [まとめ](#まとめ)

## この記事の対象読者

- 「Google Cloud Console ってそもそも何？」という人
- 無料トライアル・有料アカウント・無料枠の違いがよく分からない人
...
ブラウザで [console.cloud.google.com](https://console.cloud.google.com/) にアクセスして使う。150 以上のサービスを GUI で操作できる。
...
| **Gemini API で画像を生成したい** | 画像生成は全モデル有料。Cloud Console で課金設定を行う。詳細は [画像生成の記事](/articles/gemini-api-image-generation-guide) を参照 |
...
| **大量のデータを分析したい** | BigQuery で SQL を使ってデータ分析ができる（1 TB/月 まで無料） |
...
以下は **有料アカウントにアップグレードしないと利用できない** 機能だ。「自分に関係あるか？」の視点で整理する。
...
| サービス | 無料枠 | どんなサービスか |
|---|---|---|
| **Gemini API（テキスト生成）** | 2.5 Pro: 5 RPM・25 RPD / Flash: 15 RPM・500 RPD | AI にテキスト生成・要約・翻訳などを頼める |
| **Google AI Studio** | テキスト生成は無料。**画像生成は有料のみ** | Gemini API をブラウザ上で試せるツール |
| **BigQuery** | 1 TB クエリ/月、10 GB ストレージ | 大量データを SQL で分析できるサービス |
| **Cloud Natural Language API** | 5,000 ユニット/月 | テキストから感情・エンティティを抽出 |
...
| サービス | 無料枠 | どんなサービスか |
|---|---|---|
| **Cloud Storage** | 5 GB/月（米国リージョン） | ファイルをクラウドに保存・共有 |
| **Cloud Run** | 200 万リクエスト/月 | Web アプリやAPIをサーバーレスで動かす |
| **Firestore** | 1 GB ストレージ、5 万読取/日 | モバイル・Web アプリ向けのデータベース |
| **Compute Engine** | e2-micro VM 1 台/月（米国リージョン） | 小さな仮想サーバーを 1 台無料で動かせる |
...
| 無料トライアルが終わったらどうなる？ | リソースが停止されるだけ。放置しても課金されない |
| 有料アカウントにしたら戻せる？ | **戻せない。** 不要になったら請求先アカウントを閉鎖する |
| 使いすぎたら自動で止まる？ | **止まらない。** 予算アラートは自分で設定が必要。設定しても自動停止はしない |
| Gemini API で画像生成するには？ | 有料課金が必須。[画像生成の記事](/articles/gemini-api-image-generation-guide)で詳しく解説 |
