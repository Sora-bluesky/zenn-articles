...
この質問をされたとき、あなたは何と答えますか？
Notion？純正メモ？Evernote？

僕は今、この質問に対して「Obsidian」と答えるようになりました。
しかも、Claude Codeと組み合わせて。

なぜか。
それは、Obsidianに書いたメモを、Claude Codeが直接読み取ってくれるようになったからです。

「Obsidianに書いてある先月の会議メモを要約して」

この一言で、AIが勝手にメモを取得して、要約を作成してくれる。
コピペも、ファイルアップロードも不要。

しかも、iCloud経由でiPhoneと同期すれば、移動中にスマホでメモ → 帰宅後にAIで深掘り、という流れが自然にできる。
