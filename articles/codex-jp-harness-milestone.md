...
```toml
[mcp_servers.jp_lint]
# <repo> はリポジトリをクローンした絶対パス（install.ps1 / install.sh が自動で埋める）
...
前提: `uv` + `git` + Codex CLI（`~/.codex/` が存在する）が揃っている環境。Windows は PowerShell 7+、macOS / Linux / Git Bash on Windows は標準の bash を使う。
...
手順:
1. 適当な作業ディレクトリに git clone する（例: Windows なら Documents\Projects\apps\、macOS/Linux なら ~/Projects/）
2. リポジトリ内で `uv sync` を実行する（uv 未導入なら導入から）
3. Windows なら `pwsh scripts\install.ps1 -AppendAgentsRule`、
   macOS / Linux / Git Bash なら `bash scripts/install.sh --append-agents-rule` を実行する
   （config.toml への MCP 登録、AGENTS.md への品質ゲート規約追記、skill の自動配置を一括で行う）
...
インストーラーは次の 3 つを実行する。再インストールしても、既存の登録を書き直すだけなので安全に動く。

- `~/.codex/config.toml` に `[mcp_servers.jp_lint]` を追記（venv の Python の絶対パスを自動で検出して埋め込む）
- `-AppendAgentsRule` / `--append-agents-rule` を付けて実行したとき、`~/.codex/AGENTS.md` に `config/agents_rule.md` の本文を追記する
- `~/.codex/skills/jp-harness-tune/SKILL.md` を配置する（v0.2.2〜。利用者が手で書き換えていれば SHA-256 で比較して上書きをスキップする。不要なら `-SkipSkill` / `--skip-skill` で外せる）

`AGENTS.md` に独自のルールがあって自動追記されたくない場合は、`-AppendAgentsRule` / `--append-agents-rule` を外し、`config/agents_rule.md` の内容を手で貼り付ければよい。
...
コードは [`Sora-bluesky/codex-jp-harness`](https://github.com/Sora-bluesky/codex-jp-harness) に置いてある。導入は上のパターン A（Codex に丸投げ）かパターン B（手動）のどちらかで 1 分ほど。Windows / macOS / Linux（Git Bash on Windows 含む）のいずれでも動く。

最後に。OpenAI の日本語対応を誰よりも早く迎え入れて、このリポを役目終わりにする日を楽しみにしている。その日まで、ずんだもんには毎晩喋ってもらう。

## v0.2.x で入った拡張（2026-04-19 時点）

記事公開後、自分で毎日使っているうちに「12 語だと足りない」「このプロジェクトでは `slice` を公式用語として使っているから緩めたい」という場面が出てきた。全部を 1 つの yaml に詰め込むと、他の人の事情と衝突する。そこで v0.2.0 で 4 つを足し、v0.2.1 と v0.2.2 で細部を整えた。
...
### install 一発で skill まで入る（v0.2.2）

v0.2.1 までは skill を使いたい人に「`~/.codex/skills/jp-harness-tune/SKILL.md` を手でコピーしてください」とお願いしていた。自分で書いておいて面倒だったので、v0.2.2 で `install.ps1` / `install.sh` が配置まで面倒を見るようにした。
...
|---|---|
| 未存在 | ファイルを配置する |
| 一致（SHA-256 が bit-identical） | 上書きするが実質 no-op |
| 不一致（利用者が手編集している） | **上書きをスキップ**して stderr に警告 |

ps1 と sh の 2 本を保守することになって、同じバグを 2 回踏んで 2 回直すという素朴な苦労もついてきたが、これで `git pull` → `install` 再実行で手元のカスタムが潰れる事故は起きなくなった。ハッシュ計算は `sha256sum` → `shasum -a 256` → `certutil -hashfile ... SHA256` の 3 段フォールバックで、環境差を吸収している。

配置を明示的に避けたい場合は `-SkipSkill` / `--skip-skill` で外せる。

---

v0.2.0 / v0.2.1 / v0.2.2 の各機能の背景は、GitHub の [Release v0.2.0](https://github.com/Sora-bluesky/codex-jp-harness/releases/tag/v0.2.0)、[Release v0.2.1](https://github.com/Sora-bluesky/codex-jp-harness/releases/tag/v0.2.1)、[Release v0.2.2](https://github.com/Sora-bluesky/codex-jp-harness/releases/tag/v0.2.2) にも書いてある。
