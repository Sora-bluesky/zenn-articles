...
:::message alert
**2026年2月 Google Antigravity BAN事件**
2026年2月22-23日、OpenClaw経由でGoogle Antigravityを利用していたユーザーのGoogleアカウント全体（Gmail、YouTube、Workspace含む）が警告なしに停止された。月額$249.99のAI Ultraプラン加入者も対象。開発者のSteinberger氏はAntigravity対応をOpenClawから削除する意向を表明している。**Google AIモデルの利用は現時点で推奨しない。**
:::
...
OpenClawは、オーストリアの開発者 Peter Steinberger 氏が作ったオープンソースの自律型AIパーソナルアシスタント。Steinberger氏は2026年2月14日にOpenAI入社を発表。OpenClawはオープンソース財団に移管予定（OpenAIが支援）。名前が2回変わっている。
...
| setup-token | Claude Pro/Max契約者 | サブスクリプション内 | 使用不可（2026年1月9日ブロック済み） |
...
| モデル | 入力 | 出力 | 用途 |
|--------|------|------|------|
| Claude Opus 4.6 | $15/百万トークン | $75/百万トークン | 最高性能 |
...
:::message alert
v2026.1.29のアップデートで、Gateway認証なし（auth mode "none"）は廃止された。トークンまたはパスワードによる認証が必須。v2026.2.13以降、パブリックIPからのアクセスにはBearer Token必須となった。
:::
