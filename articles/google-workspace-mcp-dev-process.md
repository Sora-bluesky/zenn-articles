...
36個のMCP（AIに外部サービスを操作させるための仕組み）ツール、456通りの自動テスト、6つのGoogleサービスとの連携。全てClaude Codeとの共同作業で作った。プログラミングの実務経験がない一級建築士が、どう開発を進めたのか。
...
一般的なAIチャットとの違いは、ファイルを直接操作できること。コードの作成、編集、テストの実行、git（バージョン管理ツール）の操作まで自分でやる。開発者向けのツールだが、作りたいものを言語化できれば、実装はClaudeが引き受けてくれる。
...
各フェーズの流れはほぼ同じだった。設計書を作る。実装する。テスト（ツールが正しく動くかを自動で確認するプログラム）を通す。レビューする。この4つの繰り返し。

Claude Codeの中にサブエージェントという仕組みがある。1人のAIに全部任せるのではなく、設計担当・実装担当・品質チェック担当を別々に呼んで分業させるイメージだ。私が直接コードに触るのではなく、それぞれの専門AIに指示を出して結果を確認する。

---

## 設計で決めたこと

### 試し実行モード（dryRun）：実行前に必ず確認する
...
1つ目は要件を伝えること。Gmailの送信ツールがほしい、試し実行モードはデフォルトで、セキュリティ対策も入れてくれ。こういう指示を出す。
...
MCPの仕様書は読んだし、Googleの技術ドキュメントも確認した。既存のMCPサーバーがどう実装されているかも調べている。Googleに許可してもらう権限の範囲で、カレンダーの予定だけでなくカレンダー全体へのアクセスが必要だとわかったのも、自分で調べた結果だ。
