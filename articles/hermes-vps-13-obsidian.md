...
![母艦の画面を左右に並べた様子。左はWindowsエクスプローラーでiCloud Drive>Obsidian>MainVault配下のフォルダ列(.obsidian/_archive/80_Areas/Agent Skills/Agentic Workflow/AI-News/ChangeLogs/DailyNotes/Family/Journal/Kenzai-AI/Learning)、右はObsidianアプリの新規タブ画面で左サイドバーに同じMainVaultの同じフォルダ群が表示されている](/images/hermes-vps/hermes-vps-13-vault-location.png)
...
```bash
mkdir -p /home/admin/hermes-vault
```

![GNU nano 8.7.1で`/home/admin/.hermes/.env`を開いた画面。TELEGRAM/DISCORD/HERMES_DASHBOARD/WEBHOOK等のtoken値はすべて黒塗りマスクされ、末尾の追記行`OBSIDIAN_VAULT_PATH=/home/admin/hermes-vault`が赤枠で示されている。下部にnanoの日本語キーバインドヘルプ(^Gヘルプ・^O書き込み・^F検索など)](/images/hermes-vps/hermes-vps-13-env-nano-after.png)

### 反映確認と再起動

```bash
grep OBSIDIAN_VAULT_PATH ~/.hermes/.env
# → OBSIDIAN_VAULT_PATH=/home/admin/hermes-vault が出ればOK

systemctl --user restart hermes-gateway          # 反映に再起動が必要
systemctl --user status hermes-gateway           # active (running)
```

![ターミナルで`grep OBSIDIAN_VAULT_PATH ~/.hermes/.env`を実行した結果。プロンプトのホスト名は黒塗りマスク済で、出力行`OBSIDIAN_VAULT_PATH=/home/admin/hermes-vault`が赤枠で示されている](/images/hermes-vps/hermes-vps-13-env-grep.png)
...
# OBSIDIAN_VAULT_PATH=/home/admin/hermes-vault は既に設定済み(§7)
```
...
- `~/.hermes/.env`に`OBSIDIAN_VAULT_PATH=/home/admin/hermes-vault`を1行追加し、Hermesを再起動した
