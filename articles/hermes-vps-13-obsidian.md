...
- [OBSIDIAN_VAULT_PATHの設定](#obsidian_vault_pathの設定)
- [4経路の整理](#4経路の整理)
...
| 3エージェント共有 | VaultはClaude Code/Codexの個別管理 | 同じVaultをHermes/Claude Code/Codexの3つで共用(本格実装は第19回予定) |
| 配布対象 | USER.md/MEMORY.mdは個人所有で配布除外 | Vaultも同じく個人所有で配布除外(第36回Profile Distributionsで詳述) |
...
| bundled skill | Hermes本体に最初から入っているskill。`obsidian`はその1つで、追加インストール不要 |
| `OBSIDIAN_VAULT_PATH` | HermesがどのVaultを読むかを指定する環境変数。`~/.hermes/.env`に1行書くだけで有効になる |
...
第IV部全体(第12回〜第18回相当)の境界を1分で理解するための4比喩を置いておく。これを頭の地図にしておくと、本回も次回以降も「あ、これは付箋の話か。書庫の話か。手順書の話か」と即座に判別できる。
...
| 📋 手順書 | Skills | 「こう動く」のレシピ・状況に応じて取り出す | 第10回・第17回予定 |
| 🧹 棚卸し | Cron | 定期的に整える・古いものを片付ける・新しいものを足す | 第9回・第18回予定(Curator) |

連載の回数は変わる可能性がある。最新の計画書は完全構築ガイドを都度参照する。

第12回(付箋)→第13回(書庫)→第14回予定(過去会話の検索=書庫の検索)→第16回予定(人格=書庫を読む側の癖)、と並ぶ。本回(書庫)は付箋(第12回)の隣に置く分担の話だ。
...
<!-- 撮影後にここに画像挿入: hermes-vps-13-architecture-diagram.png(該当時) -->

```text
┌──────────────────────────────────────────────┐  ┌────────────────────────┐
│ Hermes Agent 本体(VPS内)                    │  │ Obsidian Vault         │
│                                              │  │ (母艦ローカル・git同期)│
│ 第1層 Markdown(私のこと)                   │  │                        │
│   ├ USER.md   (1,375字・人物カード)          │  │ Notes/                 │
│   ├ MEMORY.md (2,200字・環境ファクト)        │  │   ├ AIメモ.md          │
│   └ SOUL.md   (人格・第16回予定で扱う)       │  │   ├ プロジェクトX.md   │
│                                              │  │   ├ Hermes実機ログ.md  │
│ 第2層 SQLite(過去会話)                     │  │   └ 読書メモ.md        │
│   └ state.db  (第14回予定 Session Search)    │  │                        │
│                                              │  │ [[Wikilink]]で         │
│ 第3層 外部provider(8つ・本連載は使わない)  │  │ 相互参照               │
└──────────────────────────────────────────────┘  └────────────────────────┘
                       │                                       ▲
                       │  bundled obsidian skill経由           │
                       │  (search_files/read_file/write_file)  │
                       └───────────────────────────────────────┘
                                       │
                                       │ OBSIDIAN_VAULT_PATH環境変数
                                       │ (~/.hermes/.envに1行)
                                       ▼
                                  Vaultパスを解決
```
...
| `OBSIDIAN_VAULT_PATH`環境変数 | どのVaultを読むかを指定する1行設定 | 設定する |
...
<!-- 撮影後にここに画像挿入: hermes-vps-13-version-check.png -->
...
<!-- 撮影後にここに画像挿入: hermes-vps-13-vault-location.png -->

このあとの`OBSIDIAN_VAULT_PATH`設定では「VPS側のどこにVaultを置くか」のパスを書く。母艦のパスをそのまま転記するわけではない。母艦とVPSのVaultを揃えるのは§11で扱うgit同期の役目だ。

## bundled obsidian skillの確認

Hermes本体には`obsidian` skillが最初から入っている(bundled)。追加インストールは要らない。まずskillが存在することを確認し、SKILL.mdの中身を読んで動作の前提を把握する。
...
出力の中に`obsidian`(または`note-taking-obsidian`のようなフルネーム)があれば配置済みだ。Hermes v0.17.0では`skills/bundled/note-taking/`配下にあり、デフォルトで有効になっている。

<!-- 撮影後にここに画像挿入: hermes-vps-13-skills-list-obsidian.png -->

### SKILL.mdの中身を確認する

```bash
hermes skills show obsidian
# あるいは
cat ~/hermes-agent/skills/bundled/note-taking/note-taking-obsidian.md
```
...
## OBSIDIAN_VAULT_PATHの設定

`~/.hermes/.env`に1行追加するだけでVault連携が有効になる。設定後はHermes本体の再起動が必要だ。

### .envファイルの編集

VPSにSSHで入った状態でnanoを開く。

```bash
nano ~/.hermes/.env
```

ファイル末尾に1行追加する。VPS側に置くVaultフォルダの絶対パスを書く。

```text
OBSIDIAN_VAULT_PATH=/home/admin/vault
```

保存して終了(Ctrl+O→Enter→Ctrl+X)。実体のVaultフォルダもこのタイミングで作っておく。中身は空のままでいい。
...
# → OBSIDIAN_VAULT_PATH=/home/admin/vault が出ればOK
...
| `OBSIDIAN_VAULT_PATH`環境変数 | どのVaultを読むかを指定する | §7で設定済み |
...
```text
Hermesテストノートを作って。タイトルは「Hermes動作テスト」、本文に「2026年6月20日、第13回でObsidian連携を試した」と書いて、Vaultに保存して。
```

<!-- 撮影後にここに画像挿入: hermes-vps-13-telegram-save-request.png -->

エージェントはSKILL.mdの手順に従って、内部で`write_file`ツールを呼びVault内に.mdファイルを書き込む。Telegramには「保存しました」のような自然文で応答が返ってくる。応答の本文にVaultパスとファイル名が含まれていれば成功だ。
...
```bash
ls -la /home/admin/vault/
# Hermes動作テスト.md のようなファイルが増えている
cat /home/admin/vault/Hermes動作テスト.md
```

<!-- 撮影後にここに画像挿入: hermes-vps-13-telegram-skill-response.png -->
...
5項目テンプレの有無で、同じ題材のノートがどう変わるかを並べてみる。

<!-- 撮影後にここに画像挿入: hermes-vps-13-title-before-after.png -->
...
# OBSIDIAN_VAULT_PATH=/home/admin/vault は既に設定済み(§7)
```
...
- `~/.hermes/.env`に`OBSIDIAN_VAULT_PATH=/home/admin/vault`を1行追加し、Hermesを再起動した
...
| ← 前の回 | 次の回 → |
|---|---|
| 第12回 Hermes Agentに自分のことを覚えさせる(近日公開) | 第14回 Hermes Agentに過去の会話を検索させる方法(近日公開) |
...
| 症状 | 原因 | 対処 |
|---|---|---|
| 「Vaultが見つかりません」 | `OBSIDIAN_VAULT_PATH`未設定または誤ったパス | `grep OBSIDIAN_VAULT_PATH ~/.hermes/.env`で確認・`ls $(grep OBSIDIAN_VAULT_PATH ~/.hermes/.env \| cut -d= -f2)`で実在確認 |
| `hermes memory setup --provider obsidian`が`not found.`になる | そのコマンドはv0.17.0実機に存在しない | VaultはMemory providerではない。`OBSIDIAN_VAULT_PATH`(§7)+bundled obsidian skill(§6)で連携する |
...
| 母艦のObsidianとVPSのVaultが食い違う | git同期し忘れ | 編集後は両側で`git pull`→`git push`を習慣化する(将来的に第18回予定のCurator+cronで自動化) |
...
hermes skills show obsidian                     # SKILL.md表示

# Vaultパス設定
nano ~/.hermes/.env                             # OBSIDIAN_VAULT_PATH=... を1行追加
grep OBSIDIAN_VAULT_PATH ~/.hermes/.env         # 反映確認
systemctl --user restart hermes-gateway         # Hermes再起動

# 動作確認
ls -la /home/admin/vault/                       # Vault内ファイル一覧
cat /home/admin/vault/<ノート名>.md             # 中身確認
...
- **secure login(Dashboard)**:Dashboardにtoken必須endpointが導入された。本回では使わないが、第18回予定のCuratorでVault整理を自動化するときに関連してくる。
...
| 項目 | 引用元 |
|---|---|
| bundled `obsidian` skill仕様 | [SKILL.md (note-taking-obsidian)](https://github.com/NousResearch/hermes-agent/blob/main/skills/bundled/note-taking/note-taking-obsidian.md) |
