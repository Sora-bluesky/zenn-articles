...
:::message
**この記事の対象読者**
- Claude Code をインストール済みで、基本操作に慣れている人
- 外部ツールとの連携に興味がある人
- MCPの設定方法が分からず困っている人

Claude Code のインストールがまだの場合は、先に[インストールガイド](claude-code-windows-install-guide)を参照してほしい。
:::
...
MCPで接続される外部サービスのことを **MCPサーバー** と呼ぶ。Claude Code が「クライアント」、GitHub や Notion 側のプログラムが「サーバー」だ。

:::message
**MCPの公式情報**
- [English: Model Context Protocol](https://modelcontextprotocol.io/)
- MCPはAnthropic社が開発したオープンソースの標準仕様で、Claude Code 以外のAIツールでも採用が進んでいる
:::
...
URLを1つ指定するだけで接続できる。ローカルに何もインストールする必要がなく、最もシンプルだ。

GitHub、Notion、Sentry、Stripe など、主要サービスの多くがHTTP方式に対応している。
...
`npx` コマンドで起動するMCPサーバーは、Windows環境では `cmd /c` ラッパーが必要だ。これを付けないと正常に起動しない。
...
MCPサーバーの追加は `claude mcp add` コマンドで行う。

```bash
claude mcp add [オプション] サーバー名 [-- コマンド 引数...]
```

**重要なルール**: すべてのオプション（`--transport`、`--scope`、`--env` など）は **サーバー名の前** に書く。`--` の後ろに書いたものはMCPサーバーに渡されるコマンド引数として扱われる。
...
コマンドを実行すると、ブラウザが開いて認証画面が表示される。画面の指示に従って認証すれば完了だ。
...
サーバーを追加したら、**Claude Code を再起動**してから確認する。
...
MCPサーバーの設定には「どの範囲で有効にするか」を指定する **スコープ** がある。
...
:::message
**`~/.claude.json` とは**
`~`（チルダ）は「ユーザーのホームフォルダ」を意味する記号だ。Windows の場合は `C:\Users\[ユーザー名]` に相当する。つまり `~/.claude.json` は `C:\Users\[ユーザー名]\.claude.json` のことだ。
:::
...
MCPサーバーによっては、APIキーやトークンなどの認証情報が必要だ。管理方法は接続方式によって異なる。

### HTTP方式：OAuth認証（自動）

GitHub や Notion などのHTTP方式サーバーは、**OAuth 2.0 認証** を使う。初回接続時にブラウザが開き、画面の指示に従って認証するだけだ。APIキーを手動で管理する必要はない。

認証が必要になると、Claude Code が自動的にブラウザを開く。認証完了後は `/mcp` コマンドで接続状態を確認できる。

### stdio方式：環境変数で渡す

stdio方式のサーバーでは、`--env` オプションで環境変数として認証情報を渡す。

```bash
claude mcp add --transport stdio --env API_KEY=xxx my-server -- npx -y @some/package
```

複数の環境変数が必要な場合は、`--env` を繰り返す。
...
チーム共有用の `.mcp.json` では、認証情報をハードコードせずに環境変数を参照できる。
...
:::message alert
**APIキーの取り扱い注意**
- APIキーはパスワードと同じだ。テキストファイルに保存して放置したり、チャットで共有したりしないこと
- `.mcp.json` にAPIキーを直接書かないこと（`${VAR}` 形式で環境変数を参照する）
- 誤ってGitにコミットした場合は、APIキーを再生成すること
:::
...
以下は代表的なMCPサーバーの一覧だ。HTTP方式が使えるものはHTTP方式で接続するのが最も簡単だ。
...
初回実行時にブラウザで認証画面が開く。認証するだけで接続完了だ。
...
**よくある混乱**: セッション中に `claude mcp add` で新しいサーバーを追加しても、`/mcp` には反映されない。設定ファイルには書き込まれるが、実際の接続はClaude Code の**起動時にのみ**行われるためだ。追加後は必ず **Claude Code を再起動** すること。

### サーバーの削除

不要になったMCPサーバーは `remove` で削除できる。
...
MCPサーバーが提供する個々の機能（ツール）に対して、権限を細かく制御できる。

例えば、「GitHub の Issue は読みたいけど、勝手に閉じられたくない」という場合、特定のツールをブロックできる。

### 権限設定の方法

`settings.json` の `permissions.deny` に、ブロックしたいツールを追加する。

ツール名は `mcp__サーバー名__ツール名` というパターンだ。
...
この設定により、Claude Code は GitHub の Issue を閉じたりブランチを削除したりする操作ができなくなる。読み取りは引き続き可能だ。

### ツール名の確認方法

接続済みのMCPサーバーが提供するツール名は、`/mcp` コマンドで確認できる。サーバー名をクリックすると、利用可能なツールの一覧が表示される。

:::message
権限設定の詳細は、[権限設定の記事](claude-code-permissions)で解説している。
:::
...
チームで同じMCPサーバーを使いたい場合は、プロジェクトルートに `.mcp.json` を作成してGitにコミットする。

### 設定方法

`--scope project` を指定すると、`.mcp.json` に設定が書き込まれる。

```bash
claude mcp add --transport http --scope project github https://api.githubcopilot.com/mcp/
```

これにより、プロジェクトルートに `.mcp.json` が作成される。
...
`.mcp.json` で定義されたMCPサーバーは、初回使用時に**承認プロンプト**が表示される。チームメンバーが意図せずMCPサーバーに接続されることはない。
