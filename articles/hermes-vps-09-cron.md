...
実際に登録して動かすと、Telegramにこんな配信が届く。例の世田谷区を自分の地域に置き換えれば、同じ形式で届く。

![evening-tomorrowジョブの配信例。明日の天気と、その日に応じたごみ収集日がTelegramに届いた画面](/images/hermes-vps/hermes-vps-09-cron-evening-result.png)
...
| ← 前の回 | 次の回 → |
|---|---|
| 第8回 Web Dashboardで設定を見える化する | 第10回 Skillsに手順を覚えさせる(近日公開) |
