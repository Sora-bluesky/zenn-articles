---
title: "【第9回】Hermes Agentが朝から話しかけてくる──Cronで毎朝の定型タスクを任せる"
emoji: "🤖"
type: "tech"
...
第9回はここに「自分から動く」を足す。HermesのCron機能を使って、たとえば毎朝7時に今日のニュースとX上の話題を要約してTelegramに届ける、といった定型仕事を任せる。24時間VPSに置いた意味が、ここで一気に出る。
...
- ニュースやX検索を使うジョブを作るなら、第5回でGrok(xai-oauth)を登録済み(X検索=x_searchとWeb検索の本格設定は第11回)
...
![ターミナルでhermes cron addを実行し、Created jobとjob_id・morning-news・スケジュール・次回実行時刻が表示された画面](/images/hermes-vps/hermes-vps-09-cron-add.png)
...
![ターミナルでhermes cron runを実行し、Triggered jobと「次のtickで実行する」旨が表示された画面](/images/hermes-vps/hermes-vps-09-cron-run.png)

![少し待つと、Telegramのbotにニュース要約が届いた画面。項目ごとに出典URLが付いている](/images/hermes-vps/hermes-vps-09-cron-result.png)
...
![ターミナルでhermes cron listを実行し、morning-newsが1件active状態でjob_id・スケジュール・配信先telegramまで表示された画面](/images/hermes-vps/hermes-vps-09-cron-list.png)

![hermes cron pauseでPaused、続けてhermes cron resumeでResumedとなり、次回実行時刻が戻った画面](/images/hermes-vps/hermes-vps-09-cron-pause-resume.png)
...
```text
X上で最近議論になっているAI関連の話題を5件選び(いいね・RT等の数値は問わない)、
...
実際に登録して動かすと、Telegramにこんな配信が届く。下は筆者の環境での実行例で、市区町村名はマスクしている。例の世田谷区を自分の地域に置き換えれば、同じ形式で届く。

![evening-tomorrowジョブの配信例。明日の天気と、その日に応じたごみ収集日がTelegramに届いた画面(市区町村名はマスク)](/images/hermes-vps/hermes-vps-09-cron-evening-result.png)
...
第10回はHermes Agent Skillsで「自分用の手順を覚えさせる」回だ。毎回同じ長いプロンプトをコピペするのは面倒なので、よく使う手順をSkillにまとめてしまう。Skill化したものはCronからも呼べるので、第9回の長いプロンプトが短い呼び出しひとつで済むようになる。

---

| ← 前の回 | 次の回 → |
|---|---|
| 第8回 Web Dashboardで設定する | 第10回 Skillsに手順を覚えさせる(近日公開) |
...
| 検索を使うジョブが失敗する | 第5回でGrok(xai-oauth)を登録済みか、x_searchとWeb検索の設定(第11回)が済んでいるかを確認 |
