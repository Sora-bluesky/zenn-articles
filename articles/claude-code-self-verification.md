---
title: "Claude Codeに「本当に合ってる？」と自問させたら精度が上がった話"
emoji: "✅"
type: "tech"
...
:::message
**ブラウザの自動操作ツールを使う場合**
Playwright などのブラウザ自動操作ツールをClaude Codeと組み合わせると、スクリーンショット撮影→比較→修正のサイクルを自動化できる。導入は別記事で解説予定。
:::
...
```
変更後に npm test（プロジェクトのテストを一括実行するコマンド）を実行して、全てのテストが通ることを確認して
```

### リンター（コード品質チェック）

```
変更後に npm run lint（コードの書き方ルール違反を自動検出するコマンド）を実行して、エラーがないことを確認して
```

### 型チェック

```
変更後に tsc --noEmit（TypeScriptの型チェックだけ実行し、ファイルは出力しないコマンド）を実行して、型エラーがないことを確認して
```
...
その場合は、**手動で確認するポイントをClaudeに列挙させる**。
