...
- 第11回Hermes Agentに最新情報を自動取得させる方法
...
:::message alert
本節で扱う運用例(evening-reflection)は2026-06-19にVPSで稼働を開始したばかりで、運用検証中だ。撮影と本文の最終化は7日運用後に行う。仕組みの説明としてはここまで形にできているので、設計の参考として読んでほしい。
:::
...
Hermes本体は次に動いたタイミングで私の返信を通常のメッセージとして受け取り、`memory` toolで`~/.hermes/memories/USER.md`に「家族時間=娘の進路相談・聞き手側を意識して成功」のような形で追記する。即時ではない。書き込みが完了するのは翌セッション開始時のfrozen snapshotからだ。
...
翌朝のHermesは「昨日のそら」を覚えている。「昨日は娘の進路相談で聞き手側を意識して成功した、と覚えています」と前提を引いて記事相談に乗ってくる。
...
| ← 前の回 | 次の回 → |
|---|---|
| [第11回Hermes Agentに最新情報を自動取得させる方法](https://zenn.dev/sora_biz/articles/hermes-vps-11-web-search) | 第13回Hermes AgentとObsidianを連携して知識を共有する(近日公開) |
