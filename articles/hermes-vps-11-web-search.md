---
title: "【第11回】気になる情報を自分で探し回るな。Hermes Agentがネットで調べて要点だけまとめてくれる"
emoji: "🔎"
type: "tech"
...
2026-08-03のv0.20.0で、最初から入っているスキル(bundled)としてgrounded-citationsが追加された。検索や本文抽出に基づく回答へ番号付きの引用とSources一覧を機械的に付け、fact-checkモードでは引用文が取得ページの本文に逐語一致しない限り証拠として認めない。「それらしいが出典が怪しい」回答と「出典まで確認できる」回答を区別できるようになるので、本回のように出典URLを義務づける運用と相性がいい。有効化と使い方はリポジトリの公式ドキュメントに任せる。
:::
...
| ← 前の回 | 次の回 → |
|---|---|
| [第10回 毎回教えるな。Hermes Agentは使えば使うほど自分で賢くなる](https://zenn.dev/sora_biz/articles/hermes-vps-10-skills) | [第12回 好みを毎回言うな。Hermes AgentはMemoryで覚えている](https://zenn.dev/sora_biz/articles/hermes-vps-12-memory) |
