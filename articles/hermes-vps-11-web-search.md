...
![cat config.yamlのweb:セクション。backend: ddgsで、search_backend/extract_backendが空の初期状態(ホスト名/IPはマスク)](/images/hermes-vps/hermes-vps-11-config-web-before.png)
...
![mkdirで~/searxngを作り、nanoでdocker-compose.ymlを開いた直後の画面(ホスト名/IPはマスク)](/images/hermes-vps/hermes-vps-11-searxng-compose-edit.png)
...
![APIキー発行画面。fc-で始まるキーが表示された画面(キー本体はマスク)](/images/hermes-vps/hermes-vps-11-firecrawl-apikey.png)
...
![1Passwordの保管庫Hermes-Prodに新規アイテム「Firecrawl」を作りAPIキーを保存した画面(キー本体はマスク)](/images/hermes-vps/hermes-vps-11-firecrawl-1password.png)
...
![Telegramで検索を依頼し、3件以上のURLと一行要約が返ってきた画面(自分の表示名/ハンドル/chat IDはマスク)](/images/hermes-vps/hermes-vps-11-telegram-search.png)
...
![hermes doctorの出力に✓ xAI OAuth (logged in)と✓ x_searchが並び、config.yamlのx_searchセクションも見える画面(ホスト名/IPはマスク)](/images/hermes-vps/hermes-vps-11-doctor-xsearch.png)
...
![x_searchでXの議論・反応の要約と投稿URLが返ってきた画面。いいね/RT等の数値は出ていない(自分のハンドル/bot名/chat IDはマスク)](/images/hermes-vps/hermes-vps-11-xsearch-result.png)
...
![Telegramで/morning_newsを送り、記事URLとX投稿URLのハイブリッド結果が届いた画面。末尾の取得状況に「X検索: 使用(x_search)」と出て、本文のX投稿URLと一致している(自分のハンドル/chat IDはマスク)](/images/hermes-vps/hermes-vps-11-morning-news-hybrid.png)
