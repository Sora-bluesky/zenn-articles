...
- **第8回**(本記事)──Hermes Agentをブラウザの管制室から操る──Web Dashboardで設定を見える化する
...
- 第12回──家の余ったPCをLinux常駐GPUサーバーにする──VPSの手足を生やす
...
第7回までの僕は、設定を変えるたびにSSHでVPSに入って`nano ~/.hermes/config.yaml`を開いていた。ボットの有効化、APIキーの追加、`cron`ジョブの登録──ぜんぶ黒い画面の中で手で書いていた。
...
| Cronジョブ追加 | `hermes cron add`をSSHで | ブラウザの「新規」ボタン |
...
「セッション」ペインで各行の右端にある**▶ボタン**(Resume in Chat)を押すと、その会話の続きをこのチャットタブで再開できる。チャットタブは他のペインに移動しても殺されない設計(`display:none`で隠すだけ)なので、行ったり来たりしても会話が途切れない。
...
![CRONペイン(スケジュール済みジョブ一覧+各行にバッジ+PROFILEフィルタ+「新規」ボタン)](../images/hermes-vps/hermes-vps-08-dashboard-cron.png)

各ジョブの右端にアクションアイコン(実行/編集/削除等)。右上の「新規」ボタンを押すと、ブラウザだけで新しいジョブを作れるモーダルが開く。
...
:::message
詳しくは第9回「Hermes Agentが朝から話しかけてくる──Cronで毎朝の定型タスクを任せる」で、Dashboardから新規ジョブを作成→`Trigger now`で即実行→Telegram配信までを一気通貫で扱う。この回ではセクションを開いて存在を確認するだけで十分。
:::
...
上部のセグメント切替で3つのビュー(Skills/Toolsets/Hub)を行き来できる。
...
ここでひとつ重要な発見がある。**TelegramもDiscordもDisabledと表示されている**。実際には第4回・第5回でCLI設定済みで動いているのに、なぜか。
...
:::message alert
ただし、ここまではあくまで**Dashboard側の土台を有効化**したところまで。Tailscale IPは外部公開されないネットワーク内のアドレスなので、GitHub等の外部サービスから直接たたいてもらうには、**別途公開IPかTailscale Funnel/reverse proxy**等の経路が必要になる。実用例(GitHubのpushでHermesを起こす)までは、別の回で扱う予定。この回ではプラットフォーム自体を有効化するところまでを確認する。
:::

「チャンネルで土台を作って、Webhooksで個別購読を作る」=2段階の設計だ。
...
| 第4回 | (近日公開) | Hermes Agent本体をVPSに入れる |
| 第5回 | (近日公開) | Grok OAuth+Discord+承認モード |
| 第6回 | (近日公開) | systemdで常駐させる |
| 第7回 | (近日公開) | Hermes Desktopで母艦から操作 |
| **第8回** | **この記事** | **Web Dashboardで設定を見える化する** |
