---
title: "【第8回】手探りで動かすな。Hermes Agentはブラウザ1枚で中身が見える"
emoji: "🎛️"
type: "tech"
...
:::message
この連載は月1,800円ほどのVPSで、自分専用のAIエージェント(Hermes Agent)を24時間動かす実録だ。これはその第8回。全体の流れは[連載ハブ](https://zenn.dev/sora_biz/articles/hermes-vps-complete-guide)にまとめてある。
:::
...
:::message
**2026-06-28追記**:v0.17.0以降の「キー」ペインでは、上部のscroll-toナビが7つに拡張された(キー/OAUTH/プロバイダー/ツール/ゲートウェイ/設定/**カスタムキー**)。最後の「カスタムキー」が新セクションで、Hermesが標準で認識しない自分で追加した`.env`の任意の環境変数を一覧+追加できる。skillやMCP serverに渡したい独自のenv varを置く先として使う。本記事上のスクショはv0.16.0時点の画面なので5タブ4セクション表記のまま据え置く。
:::
...
:::message
**2026-07-01追記**:MCPサーバーを追加してconnected/enabled状態なのにセッションのツール一覧に出ない不具合が[PR#55514](https://github.com/NousResearch/hermes-agent/pull/55514)で修正された(元issue [#51587](https://github.com/NousResearch/hermes-agent/issues/51587))。起動が遅いMCPサーバーが「late-refresh」の対象から漏れていたのが原因。もし追加したMCPが見えないときは、v0.17.0以降に更新して`hermes-gateway`を再起動すれば表示される。
:::
...
1. **Profile Builder追加**=`config.yaml`を手で書き換えなくても、ブラウザからmodel/skills/MCP serverを選んでプロファイルを組める。複数プロファイルを1画面で管理するglobal switcherも追加。第43回(Profile管理・profile distributionsの回)で詳しく扱う予定だ。
...
3. **Subagent Watch-Windows**=委譲したサブエージェントの活動を専用ペインで並列ストリーム表示できる。第21回(Claude Code/Codex連携)で本格的に使う。
...
| ← 前の回 | 次の回 → |
|---|---|
| [第7回 SSHはもう開くな。Hermes Agentはデスクトップアプリから直接話せる](https://zenn.dev/sora_biz/articles/hermes-vps-07-desktop) | [第9回 いつもの作業を毎回自分でやるな。Hermes Agentが決めた時刻や間隔で自動でこなす](https://zenn.dev/sora_biz/articles/hermes-vps-09-cron) |
