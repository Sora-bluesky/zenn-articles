...
1. **Profile Builder追加**=`config.yaml`を手で書き換えなくても、ブラウザからmodel/skills/MCP serverを選んでプロファイルを組める。複数プロファイルを1画面で管理するglobal switcherも追加。v0.17.0で追加された機能で、本連載での専用回は未確定だ。
