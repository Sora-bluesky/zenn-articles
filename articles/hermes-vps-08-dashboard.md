...
1. **Profile Builder追加**=`config.yaml`を手で書き換えなくても、ブラウザからmodel/skills/MCP serverを選んでプロファイルを組める。複数プロファイルを1画面で管理するglobal switcherも追加。第40回(Profile管理・profile distributionsの回)で詳しく扱う予定だ。
...
3. **Subagent Watch-Windows**=委譲したサブエージェントの活動を専用ペインで並列ストリーム表示できる。第20回(Claude Code/Codex連携)で本格的に使う。
