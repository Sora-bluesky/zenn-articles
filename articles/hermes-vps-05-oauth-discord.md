---
title: "【第5回】コマンドを覚えるな。Hermes AgentはDiscordで話しかけるだけで動く"
emoji: "🤖"
type: "tech"
...
- **第5回**(本記事) コマンドを覚えるな。Hermes AgentはDiscordで話しかけるだけで動く
...
:::message alert
**2026-07-12追記**:公式ドキュメントの必要権限リストが更新され、「**公開スレッドの作成**」(Create Public Threads)が加わった。本記事の設定`auto_thread: true`がチャンネルにスレッドを立てるときに使う権限で、サーバーの既定権限で許可されていれば動くが、**上の5項目と合わせてチェックしておくのが確実**だ。すでに招待済みのbotには、Discordのサーバー設定からbotのロールに「公開スレッドの作成」を後付けすればよい(再招待は不要)。スクリーンショットは5項目時点のもの。
:::
...
:::message
**2026-07-01追記**:Discordの2000字上限で応答が途中で切れる不具合が[PR#27881](https://github.com/NousResearch/hermes-agent/pull/27881)で修正された。長い応答も自動的に分割配信されるようになり(Telegram同様のsplit-and-deliver方式)、以前のように途中で消えることはない。
:::

:::message
**2026-07-03追記(v0.18.0)**:Discordの共有channelで、allowlist外のメンバーが発言すると発言者名に`[unverified]`のプレフィックスが付くようになった(commit `8e94e8f88`)。第三者の発言をHermesが誤って本人の指示と混同しないための安全策で、Slackの同様のthreadタグ付けと対になる機能。既存の設定・手順は変わらない。
:::
...
:::message alert
**2026-07-14追記**:新規セットアップのデフォルトが`manual`から`smart`に変わった([PR#62661](https://github.com/NousResearch/hermes-agent/pull/62661))。以前のバージョンで導入した環境や、本記事どおり`hermes setup`を実行した環境では、上のコマンドの出力が下の期待出力ではなく`mode: smart`になっている場合がある。`smart`はフラグの立ったコマンドを1つずつAIが個別に評価する中間モードで、Docker backend(本シリーズ既定)では承認プロンプト自体が原則出ない挙動に変わりはない。あわせて、承認待ちの秒数を決める`timeout`の新規デフォルトも60秒から300秒(5分)に変わった。下の期待出力とスクリーンショットは`timeout: 60`のままだが、いま新しく`hermes setup`した環境では`timeout: 300`と出る。これは承認プロンプトが出たときの待ち時間が延びただけで、Docker既定では承認自体が原則出ないため、どちらの値でも手順は同じように進む。**本記事は引き続き`manual`(コマンド発火のたびに必ず人間へ確認を求める・裁量の余地なし)で固定する方針**なので、`smart`だった場合は次の手順で上書きする。
:::
...
:::message
**v0.14系のキー名整理**:正規キーは`approvals:`(複数形、トップレベル)で、子キーに`mode:`を持つ。値は`manual`(=毎回確認)/`smart`(=フラグの立ったコマンドだけAIが個別評価・2026-07-14以降の新規デフォルト)/`off`(全自動)。設定ファイルには別に`approval:`(単数形)というキーも存在するが、これは「承認判定用サブエージェントの設定」(別LLMに承認判定をさせる構成)で、承認モードそのものとは別の機能だ。本シリーズで触るのは`approvals.mode`のみ。
:::

`manual`以外になっていたら、`config.yaml`を直接書き換える(`hermes setup agent`の再実行では承認モードを選ぶ画面が無いため、値は変わらない)。

```bash
sed -i 's/^  mode: .*/  mode: manual/' ~/.hermes/config.yaml
```
...
| ← 前の回 | 次の回 → |
|---|---|
| [第4回 Hermes AgentをDockerで隔離して動かす方法](https://zenn.dev/sora_biz/articles/hermes-vps-04-install) | [第6回 気づいたら止まっている、をなくせ。Hermes Agentはsystemdでいつも動き続け、落ちてもすぐ戻る](https://zenn.dev/sora_biz/articles/hermes-vps-06-systemd) |
