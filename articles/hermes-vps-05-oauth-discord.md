...
:::message alert
**2026-07-14追記**:新規セットアップのデフォルトが`manual`から`smart`に変わった([PR#62661](https://github.com/NousResearch/hermes-agent/pull/62661))。以前のバージョンで導入した環境や、本記事どおり`hermes setup`を実行した環境では、上のコマンドの出力が下の期待出力ではなく`mode: smart`になっている場合がある。`smart`はフラグの立ったコマンドを1つずつAIが個別に評価する中間モードで、Docker backend(本シリーズ既定)では承認プロンプト自体が原則出ない挙動に変わりはない。**本記事は引き続き`manual`(コマンド発火のたびに必ず人間へ確認を求める・裁量の余地なし)で固定する方針**なので、`smart`だった場合は次の手順で上書きする。
:::
