...
winsmux は smux の「移植」ではなく、psmux 向けの再実装から始まり、現在はエージェント・オーケストレーション・プラットフォームに発展した。コマンド体系は smux 互換を維持しつつ、Orchestra（Commander / Builder / Reviewer のマルチロール構成）、Shield Harness（22 の security hooks）、Vault（DPAPI＝Windows がユーザーごとに暗号化する仕組み、を使った資格情報管理）、Mailbox（Named Pipe＝Windows のプロセス間通信機構、を使った非同期メッセージング）などを追加している。
...
v0.10.0 で winsmux は smux 互換ツールからオーケストレーション基盤に変わった。ここでは主要な追加機能を紹介する。

### Orchestra -- マルチエージェント自動起動

`.psmux-bridge.yaml` でエージェント構成を定義し、`orchestra-start`（psmux-bridge のコマンドではなく winsmux の起動スクリプト）で一括起動する。ペイン配置、エージェント起動、ラベル付け、Vault 注入が自動化される。
...
`.claude/hooks/` に 22 の JavaScript フックを配置する。プロンプトインジェクション検出、データ境界制御、Evidence Ledger（OCSF＝セキュリティイベントの標準フォーマット、に準拠した監査ログ + SHA-256 ハッシュチェーン）を自動適用する。エージェントを野放しにしないためのガードレールだ。
