---
title: "【第6回】止まるたびに立ち上げ直すな。Hermes Agentはsystemdで自分から立ち上がる"
emoji: "🤖"
type: "tech"
...
- **第6回**(本記事) 止まるたびに立ち上げ直すな。Hermes Agentはsystemdで自分から立ち上がる
...
:::message
**2026-07-01追記**:v0.17.0以降のrollingで、Telegramのpolling updater(長時間ポーリング)が黙って停止する不具合が自動復旧するようになった([#55769](https://github.com/NousResearch/hermes-agent/pull/55769))。以前は`get_me()`が正常に返る一方でメッセージ受信だけが止まる状態が起き、gateway自体は動いているのに沈黙するハマりがあった。常駐運用中に「返事が来ない時間帯がある」と感じたら、まず`systemctl --user restart hermes-gateway`で再起動すると、以降は自己検出で復旧するようになる。
:::
