---
title: "【第7回】Hermes Agentをマウス操作で動かす──公式アプリ「Hermes Desktop」を入れる"
emoji: "🤖"
type: "tech"
...
インストールしたHermes Desktopを起動したのに、アプリが立ち上がらない——という場合がある。私もこれに当たった。ChromeやEdgeは普通に開くのに、Hermes Desktopだけが起動しない。

何が悪いのか見当もつかなかったので、Claude Codeに症状を伝えて原因を突き止めてもらった。返ってきた答えはこうだ。

Hermes Desktopの土台にはElectron(中身はChromium、Chromeと同じ仕組み)が使われている。そのChromiumの「サンドボックス」という保護機能が、一部のGPU(Intel/AMD)とドライバの組み合わせで、画面を描くプロセスを巻き込んで異常終了させる(終了コード`0x80000003`、公式issue [#38216](https://github.com/NousResearch/hermes-agent/issues/38216))。ChromeやEdgeは開けるのに同じElectron系のアプリ(Hermes・Notion・Discord等)だけが起動しないのが、この問題の見分け方になる。

対処は、起動時にこのサンドボックスを切るオプション`--no-sandbox`を付けること。これもClaude Codeに頼んで、デスクトップのショートカットを用意してもらった。やっていることはシンプルで、ショートカットの「リンク先」を`...\Hermes.exe --no-sandbox`のように、実行ファイルのうしろに半角スペースと`--no-sandbox`を足すだけだ。これで起動するようになった。

直前に何度も起動を試みた後は、壊れたキャッシュが残っていることがある。その時は`%APPDATA%\Hermes\GPUCache`と`Code Cache`の2つのフォルダを消してから起動すると直る。

:::message
`--no-sandbox`が切るのは、あくまで「Hermes Desktopというアプリの画面表示プロセス」のサンドボックスだけ。第4回で設定したエージェントのコマンド実行(Dockerコンテナ隔離)とは別の話で、エージェント側の安全境界は何も変わらない。とはいえブラウザ由来の保護を1枚はがすのは事実なので、最終的にはGPUドライバを最新に更新し、`--no-sandbox`なしで起動できる状態を目指すのがよい。
:::
...
接続が成功すると、Desktopの中身がVPSのHermesに切り替わる。VPS側で動いていたセッションや設定がそのまま見える。母艦単独では止まっていたチャットも、ここからは正常に返事が返るようになる。
...
VPSに繋がって頭脳が動くようになった。ここからはDesktopアプリそのものの触り方を押さえる。ここがv0.16.0で一番厚くなった部分で、「ターミナルのコマンドを覚える」から「アプリを普通に使う」へ変わる。
...
:::message alert
**左サイドバーにすぐ出ないとき**(更新の一手間):Telegram/Discordで作った会話は、Desktopの左サイドバーにリアルタイムでは出ない(Hermes Desktopは通常の会話一覧を定期取得していないため。自動更新されるのはCronなど一部だけ)。出ないときは、Gateway設定で「**Save and reconnect**」を押す。再接続がかかり、一覧が更新される(Windowsで実機確認済み)。それでも出なければアプリを再起動する。`Ctrl+R`(ウィンドウ再読み込み)はWindowsでは効かなかった。この更新の一手間は、将来のアップデートで自動同期されるよう改善が期待される。
:::
...
| Telegram/Discordの会話が左サイドバーに出ない | リアルタイム同期されないのが既知の挙動。`Save and reconnect`またはアプリ再起動で更新する(本回「どの入口でも同じ1体のエージェント」参照) |
