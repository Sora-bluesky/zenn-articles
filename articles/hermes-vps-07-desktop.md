...
接続が成功すると、Desktopの中身がVPSのHermesに切り替わる。一番わかりやすいのは左サイドバーだ。母艦単独の時は空だったサイドバーに、VPS側のチャット履歴やCronジョブが並んでいれば、それがVPSに繋がった証拠。第4〜6回でTelegramやDiscordから送った会話、第9回で設定するCronの定期タスクなど、VPSのHermesが持っているデータがそのまま見える。
...
VPSに繋がって頭脳が動くようになった。サイドバーにチャット履歴が並んでいるのが見えていれば、準備は完了だ。ここからはDesktopアプリそのものの触り方を押さえる。ここがv0.16.0で一番厚くなった部分で、「ターミナルのコマンドを覚える」から「アプリを普通に使う」へ変わる。
