...
:::message
**2026-07-12追記**:公式のホスティング版「Hermes Agent Cloud」が公開された(ポータルでモデルとサーバーサイズを選ぶと60秒ほどで起動する)。まず触ってみるならCloudは良い入口だ。この連載はその上で、**自分のVPSに自分の城を建てる**路線を扱う。会話も記憶も秘密も自分のサーバーに残り、スキルや外部連携を根っこから自由に組めて、費用はVPS代の月1,800円ほどで固定という違いがある。Cloudで気に入ってから自分の城へ引っ越す人にも、この連載の手順はそのまま地図として使える。
:::
