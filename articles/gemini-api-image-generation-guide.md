...
emoji: "🎨"
type: "tech"
topics: ["ai", "生成ai", "画像生成", "googlecloud", "GeminiAPI"]
published: true
---
...
8. [生成画像の著作権と利用権](#生成画像の著作権と利用権)
9. [AI 生成画像の著作権に関する法的留意点](#ai-生成画像の著作権に関する法的留意点)
10. [まとめ](#まとめ)
...
| **課金体系** | Google One AI Premium 等のサブスクリプション | Google Cloud の従量課金（使った分だけ請求） |
...
**Gemini ネイティブモデル（Nano Banana ファミリー）。** 1 つのモデルでテキスト生成と画像生成の両方を行える（このような複数の入出力に対応するモデルを「マルチモーダルモデル」と呼ぶ）。
...
画像生成は有料機能のため、API キーが漏洩すると第三者に課金される恐れがある。以下の対策を推奨する。

### API キーに制限をかける

API キーを作成したら、**使用する API だけに制限をかける**ことが重要だ。制限をかけないと、そのキーで Google Cloud の全サービスにアクセスできてしまう。

| 制限の種類 | 設定内容 | 効果 |
|---|---|---|
| **API の制限** | Generative Language API のみ許可 | 他のサービス（Cloud Storage 等）への不正利用を防止 |
| **アプリケーションの制限** | 特定の IP アドレスやドメインのみ許可 | キーが漏洩しても他の場所から使えない |
...
API キーをソースコード（プログラムの中身）に直接書き込まない。環境変数（パソコンの設定値）や Secret Manager（Google Cloud の安全なキー保管庫）を使う。

| 保管方法 | 難易度 | セキュリティ |
|---|---|---|
| 環境変数 | 簡単 | 基本的な保護 |
| **Secret Manager** | やや手間 | **最も安全**（暗号化 + アクセス管理） |
...
| **機密情報の送信** | 非推奨 | データ処理に関する追加契約（Data Processing Addendum）に基づき処理 |
...
| 可視性 | **人間の目には見えない**（不可視） |
| 画質への影響 | なし |
| 耐性 | クロップ、フィルター追加、JPEG 保存などの画質劣化にも耐える |
| 除去 | できない設計 |
| 目的 | AI 生成画像の識別 |
...
- **まずは無料で試したい** → [Gemini アプリ](https://gemini.google.com/) で画像生成を体験する（API とは別物だが、品質の参考になる）
- **API で画像生成を始めたい** → Google Cloud の課金設定が必要。[課金の基礎知識](google-cloud-console-basics)を先に確認
- **課金設定の見直し** → [課金管理の記事](google-cloud-billing-check)で現在のアカウント状態を確認
...
:::message
Google AI / Google Cloud の公式ドキュメントは英語版が最新だ。ブラウザの翻訳機能で日本語に切り替えて読める。
:::
...
- [API キーに制限を追加する | Google Cloud](https://cloud.google.com/docs/authentication/api-keys?hl=ja)
- [Secret Manager の概要 | Google Cloud](https://cloud.google.com/secret-manager/docs/overview?hl=ja)
