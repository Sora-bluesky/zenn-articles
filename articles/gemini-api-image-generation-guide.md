...
> 公式料金ページにて、すべての画像生成モデルの Free tier 列に "Not available" と明記されている。
...
### Gemini ネイティブモデル（Nano Banana ファミリー）

テキスト生成と画像生成を 1 つのモデルで行える「マルチモーダルモデル」である。
...
:::message
「Nano Banana」は、Google が LMSYS Chatbot Arena に匿名で投稿した際のコードネームだった。Midjourney や Flux を上回る評価を受け、そのままブランド名として定着した。
:::
...
テキストプロンプトから画像のみを生成する専用モデルである。**英語プロンプトのみ対応**。
...
Gemini ネイティブモデル 2 種の違いを詳しく比較する。

| 比較項目 | Nano Banana | Nano Banana Pro |
|---|---|---|
| **モデル ID** | `gemini-2.5-flash-image` | `gemini-3-pro-image-preview` |
| **コンセプト** | 高速・低コスト | 高品質・プロ向け |
| **最大解像度** | 1K | **4K** |
| **思考モード** | 非対応 | **対応**（構図を推敲してから出力） |
| **Google 検索連携** | 非対応 | **対応**（リアルタイムデータに基づく生成） |
| **テキスト描画** | 基本的 | **高度**（図表・メニュー・マーケティング素材向け） |
| **多言語プロンプト** | 対応 | 対応 |
| **画像編集** | 対応 | 対応 |
| **リファレンス画像** | 対応 | 対応（最大 14 枚） |
| **Batch API** | 対応 | 対応 |

**選び方の目安：**
- **とにかく安く画像生成したい** → Nano Banana（$0.039/画像）
- **4K が必要、テキストの読みやすさが重要** → Nano Banana Pro（$0.134〜$0.24/画像）
- **コスト最優先で英語プロンプトでも構わない** → Imagen 4 Fast（$0.02/画像）

— 出典：[DeepMind - Gemini Image Flash](https://deepmind.google/models/gemini-image/flash/)、[DeepMind - Gemini Image Pro](https://deepmind.google/models/gemini-image/pro/)
...
| モデル | 無料枠 | 有料料金（1 画像） | Batch API |
|---|---|---|---|
| `gemini-2.5-flash-image` | **なし** | $0.039 | $0.0195（半額） |
| `gemini-3-pro-image-preview` | **なし** | $0.134（1K/2K）、$0.24（4K） | $0.067 / $0.12 |
| `imagen-4.0-fast-generate-001` | **なし** | $0.02 | — |
| `imagen-4.0-generate-001` | **なし** | $0.04 | — |
| `imagen-4.0-ultra-generate-001` | **なし** | $0.06 | — |

> すべての画像生成モデルの Free tier 列は "Not available" と記載されている。
> — 出典：[Gemini API Pricing](https://ai.google.dev/gemini-api/docs/pricing)
...
:::message alert
**Google AI Studio は「UI の利用自体は無料」だが、画像生成機能は Free tier では使えない。** 料金ページで明確に "Not available" と記載されており、Free tier で画像生成を試すとクォータエラーになる。「AI Studio 無料 ＝ 画像生成も無料」ではない点に注意。
...
| 知りたいこと | 答え |
|---|---|
| API で画像生成は無料でできる？ | **できない。** 全モデルの Free tier は "Not available" |
