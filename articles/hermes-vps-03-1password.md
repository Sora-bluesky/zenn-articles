...
- [そもそも何が危険なのか](#そもそも何が危険なのか)
- [LLM接続はOAuthでHermes本体に任せる](#llm接続はoauthでhermes本体に任せる)
- [では何を1Passwordで管理するか](#では何を1passwordで管理するか)
- [構成の全体像](#構成の全体像)
- [MCPを採用しない理由(意識的選択)](#mcpを採用しない理由(意識的選択))
- [事前準備](#事前準備)
- [サービスアカウントを発行する(4ステップウィザード)](#サービスアカウントを発行する(4ステップウィザード))
- [VPSにopを入れて認証する](#vpsにopを入れて認証する)
- [op runで参照解決経路を作る](#op-runで参照解決経路を作る)
