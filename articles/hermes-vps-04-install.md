...
- **第4回**(本記事)──Hermes Agentを実体としてVPSに置く(インストールとDockerサンドボックス)
...
| Telegram bot疎通 | tokenの参照経路だけ | 「テスト」と話しかけたら「テスト受信しました。」と返信される |
...
:::message
**バグ回避のためのmain運用が、本記事の最大の判断**:Hermes Agentは活発に開発されており、リリースtagには上記のような未解決バグが残ることがある。「最新tagをチェックアウトすれば安全」とは限らない。**該当のIssue/PRを確認→修正がmainにあれば main運用に切り替える**、を毎回考える必要がある。
:::

### インストール

公式は2通りの方法を提示している。本シリーズは初心者向けに**方法A(`./setup-hermes.sh`一発)**を推奨する。

#### 方法A:setup-hermes.shを使う(推奨)
...
`uv sync`は使わない。`uv sync`は`pyproject.toml`のbase dependencyだけを見て`[all]`extraを読まないため、Telegram統合等が入らない。`uv pip install -e ".[all]"`(または`./setup-hermes.sh`)が正しい。
...
| **Telegram bot token** | **BotFatherから取得した値**を入力 | ★後で事後処理(後述) |
| Allowed user IDs | 自分のTelegram user ID | @userinfobotで取得 |
...
### つまずきポイント2:Telegram bot tokenが.envに平文保存される

setup wizardの「Telegram bot token」入力で値を入れると、`~/.hermes/.env`に`TELEGRAM_BOT_TOKEN=<実値>`として**平文で書かれる**。本シリーズ方針(`op://`参照経由でディスク平文を残さない)と矛盾する。

setup完了後の事後処理:

```bash
nano ~/.hermes/.env
# TELEGRAM_BOT_TOKEN=... の行を削除して保存
```
...
### つまずきポイント3:Codex OAuthはsetup wizardで一緒に登録される

これは予想外の挙動だった。setup wizardの「Provider選択でOpenAI Codex」を選んだ段階で、**OAuth登録(device codeフロー)も同時に走る**。第4回終了時点で`hermes auth list`を確認すると:
...
そのため、本シリーズの第5回で「Codex OAuth登録」を別途やる必要はない(必要なら`hermes auth add openai-codex`を再実行することでtoken refreshできる)。第5回でやるのはGrok OAuth(SSHトンネリングが必要)と承認モード固定。
...
| `auth.json` | setup wizardのProvider選択時に作成 | OAuth token保管 | **絶対に触らない**(Hermes Agentが排他管理) |
...
Ubuntu 26.04(2026年4月リリース)が新しすぎてPlaywright未対応(2026年5月時点)。Browser tools(`browser_navigate`等)は当面使えない状態になるが、**Hermes Agent本体のテキスト応答とDuckDuckGo検索は問題なく動く**。Playwrightのバージョンアップ待ち、または旧Ubuntu(22.04/24.04)を選ぶ判断が必要。本シリーズではUbuntu 26.04のまま、Browser toolsはオプション扱いで進める。
...
### つまずきポイント6:`'NoneType' object is not iterable`が出る

第4回最大の罠がこれ。`op run -- hermes gateway`起動後、Telegramでメッセージを送ると以下のエラーで死ぬ。
...
これがHermes Agent v0.14.0の既知バグで、本記事冒頭で説明したとおり**mainブランチへの切り替え+再インストール**で解消する。リリースtag(v2026.5.16)のままでは踏む。
...
`agent/codex_runtime.py`+`agent/auxiliary_client.py`が更新されてNoneガードが入る。これ以降、Telegramへの返信が正常に動くようになる。
...
- [ ] `~/.hermes/.env`からTelegram bot token行を削除済み
...
### 4. Telegram bot tokenの`.env`平文化

setup wizard中に入力するとディスクに平文で残る。setup完了後に該当行を削除し、`op run`経由で起動する運用に一本化する。

### 5. Codex OAuthはsetup wizardで一緒に登録される
...
botにユーザーから最初のメッセージを送って chat 確立してから再起動。
...
| 本記事参照tag | [release v2026.5.16](https://github.com/NousResearch/hermes-agent/releases/tag/v2026.5.16)=v0.14.0(ただしmain運用推奨) |
...
| setup wizard構成 | [hermes_cli/setup.py](https://github.com/NousResearch/hermes-agent/blob/v2026.5.16/hermes_cli/setup.py) |
