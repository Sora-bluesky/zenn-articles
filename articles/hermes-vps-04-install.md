---
title: "【第4回】Hermes AgentをDockerで隔離して動かす方法"
emoji: "🤖"
type: "tech"
...
:::message
この連載は月1,800円ほどのVPSで、自分専用のAIエージェント(Hermes Agent)を24時間動かす実録だ。これはその第4回。全体の流れは[連載ハブ](https://zenn.dev/sora_biz/articles/hermes-vps-complete-guide)にまとめてある。
:::
...
- [出てくる用語の最低限の理解](#出てくる用語の最低限の理解)
- [なぜコマンドをDockerで隔離するのか](#なぜコマンドをdockerで隔離するのか)
- [第4回終了時点の構成図](#第4回終了時点の構成図)
- [事前準備](#事前準備)
- [Hermes Agentをインストールする](#hermes-agentをインストールする)
- [対話ウィザードで初期設定する](#対話ウィザードで初期設定する)
- [手動起動してTelegramから話しかける](#手動起動してtelegramから話しかける)
...
- **第4回**(本記事) Hermes AgentをDockerで隔離して動かす方法
...
![Telegramで「hello」送信→Hermes Agentから「Hello! How can I help?」と返信(ターミナル側はop run経由でhermes gateway起動中)](/images/hermes-vps/hermes-vps-04-telegram-success.png)
...
## 補足:2026-06-21に公式Docker imageが公開された

本記事は第三者製の`nikolaik/python-nodejs:python3.11-nodejs20`をDockerのベースイメージにしている。執筆時点で動作確認済みの安定構成で、今もそのまま使える。

2026-06-21にNousResearch公式から`nousresearch/hermes-agent`(Docker Hub)が公開された。新規読者は公式イメージから始めても、ほぼ同じ手順で動く。既存の読者は乗り換える必要はない=`nikolaik`イメージのままで本シリーズの第5回以降も問題なく繋がる。

切り替える場合は`compose.yaml`の`image:`を書き換えるだけだ。本記事は`nikolaik`での動作実績で書いているので、再撮影や本文訂正は行わない。`hermes update`での本体追従と、Docker imageの差し替えは別の話と覚えておくとよい。
...
| ← 前の回 | 次の回 → |
|---|---|
| [第3回 APIキーをそのまま書くな。Hermes Agentの秘密は1Passwordが預かる](https://zenn.dev/sora_biz/articles/hermes-vps-03-1password) | [第5回 コマンドを覚えるな。Hermes AgentはDiscordで話しかけるだけで動く](https://zenn.dev/sora_biz/articles/hermes-vps-05-oauth-discord) |
