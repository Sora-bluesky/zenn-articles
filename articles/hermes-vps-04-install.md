...
### つまずきポイント2:Telegram bot tokenが.envに平文保存される(setup完了直後の必須処理)

setup wizardの「Telegram bot token」入力では**実値を入れるしかない**(wizard側で空欄skipができない仕様)。値を入れた瞬間、`~/.hermes/.env`に`TELEGRAM_BOT_TOKEN=<実値>`として**平文で書かれる**。本シリーズ方針(`op://`参照経由でディスク平文を残さない、第3回参照)と矛盾するので、**setup完了の直後に必ず削除する**。
...
本記事冒頭の「既知バグ回避のためmainブランチで運用する」(`git checkout main && git pull`)を実施した読者は**踏まない**罠。もしリリースtag(`v2026.5.16`)のままで運用していると、`op run -- hermes gateway`起動後、Telegramでメッセージを送ると以下のエラーで死ぬ:
