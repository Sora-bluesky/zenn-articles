...
ChatGPTやClaudeを毎日使っているのに、開くたびに自分のことを一から説明し直している——そんな心当たりはないだろうか。賢いのに、こちらの事情は覚えてくれない。パソコンを閉じれば止まるし、出先のスマホからは細切れにしか頼れない。「自分のことを覚えていて、24時間こちらのために動く専属のAI」がいたら、と一度は思うはずだ。
...
- [この回の到達点](#この回の到達点)
- [そもそもHermes Agentとは何か](#そもそもhermes-agentとは何か)
- [なぜいまHermes Agentなのか](#なぜいまhermes-agentなのか)
- [第1回終了時点の構成図](#第1回終了時点の構成図)
- [XServer VPS 6GBを契約する](#xserver-vps-6gbを契約する)
- [Linuxでつまずかないための最低限の知識](#linuxでつまずかないための最低限の知識)
...
自律型AIエージェントHermes AgentをVPSに常駐させた記録だ。XServer VPS 6GB(Ubuntu 26.04)を契約し、ログイン可能な状態にして、管理ユーザー(admin)を作成し、SSH鍵認証に切り替え、rootのSSHログインを閉じるところまでを扱う。

実機で打ちながら書いたメモなので、きれいな手順書ではない。詰まった場所も含めて読んでもらいたい。

先にこの記事で何度も出てくる3つの言葉だけ説明しておく。

- **VPS**(Virtual Private Server):クラウド事業者から借りる、自分専用のLinuxマシン。電源やネット回線の管理は事業者がやってくれるので、借りた瞬間から24時間動くサーバーが手に入る
- **SSH**(Secure Shell):手元のPCから遠くのサーバーにログインして、文字でコマンドを打って操作する仕組み。画面共有ではなく、命令を打ち込んで動かす
- **root**:そのサーバーで何でもできる最高権限のユーザー。便利だが、打ち間違いがそのままシステム全体の破壊につながるため、普段使いには向かない
...
ポイントは、Hermes本体は「段取り役」であり、思考そのものは外部のAIモデルに委ねている点だ。私の環境では既存のChatGPT/CodexサブスクリプションをOAuthで繋ぎ、`openai-codex`プロバイダ(2026年5月時点での最新Codexモデル)で動かしている。xAIの最新Grokに切り替えたい場合は`xai-oauth`プロバイダを選ぶ。具体的な切り替えコマンドは第5回(Grok OAuth登録の回)で扱う。

## なぜいまHermes Agentなのか

Hermes Agentが2026年に入ってから急激に存在感を増している。数字で言うと、2026年2月25日のローンチから約3か月で**GitHubスター17万超・貢献者1,000人近く**(2026年5月時点で約171,000スター)を集め、5月10日時点で**OpenRouterの全AIエージェント中で日次トークン消費1位**(224B/日)に立った。前任者のOpenClawの186B/日を抜き、約30%のOpenClawユーザーが乗り換えたとされる(Reddit調査)。
...
「AIに話しかけて作業させる」だけなら、すでにClaude Code、Codex CLI、Cursor、Continue.devなど類似ツールは山ほどある。Hermes Agentが違うのは、**ツールではなく常駐型エージェント**として設計されている点だ。比較表で整理する。
...
そして、こうしたエージェントの良し悪しを最後に分けるのは、モデルの賢さよりも**アーキテクチャ**だ——どんなツールとスキルを持たせ、記憶をどう残し、使うほど学ぶループをどう回すか。いわば9割がアーキテクチャ、1割がAI。本連載が一貫して扱うのはその仕組みで、第2回以降を1段ずつ用意していく。
...
- **Telegram = 窓口**：手元のスマホ/PCからHermesに話しかけて応答を受ける
...
なお、この機種でなければ動かないわけではない。自宅で手足として使うマシンは、家に眠っている古いWindowsノートPCや、使わなくなったMac(IntelのMacBookでもMac miniでも)で十分まかなえる。Linuxは軽いので、最新のWindowsには重すぎる世代の機材でも快適に動く。画像生成やローカルでのAI推論まで本格的にやる場合だけRTX 30シリーズ以上のGPUが要るが、「VPSのHermesから呼び出して軽い処理を任せる相手」としてだけなら、CPUしかない古いマシンでも実用になる。そもそも自宅機は第10回で足す任意の拡張なので、第1回から第9回まではVPSだけで完結する。

自宅機の構成判断(Linux専用機として運用する選択・GPUドライバ・BIOS設定の罠)の詳細は第10回(家のPCをLinuxの常駐GPUサーバーにする回)で扱う。第1回では「自宅機=AI推論専用のLinux機」とだけ覚えておけばよい。

このシリーズでは、図の中の各要素を回ごとに実装する。第1回はVPSを契約してadminで安全にログインできるところまでを片付ける。

## XServer VPS 6GBを契約する

ここから実際の構築に入る。プロバイダは[XServer VPS](https://vps.xserver.ne.jp/)の6GBプランを選んだ（4コア / 6GB / SSD 150GB / 月1,800円前後）。なお本連載に貼るリンクは公式ページを指すもので、アフィリエイトリンクではない。

プラン選定の経緯も書いておく。最初の設計ではConoHa VPSの4GBプランを候補にしていた。Docker済みイメージですぐ動かせて、スナップショットで試行錯誤しやすいのが理由だった。ただ詰めていくと、ConoHa 4GB（更新月2,000円台）よりXServer VPS 6GB（更新月1,800円前後）のほうが、むしろ安くて容量も多いと分かった。Hermesは常駐させると地味にメモリを使うので、4GBより6GBの余裕は効いてくる。構成自体はどのプロバイダでも変わらない（プロバイダ非依存）ので、乗り換えのデメリットもない。加えて、もともとXServerに休眠状態の契約があった縁もあり、改めて6GBプランで契約し直してXServer 6GBに決めた。

海外VPS（VultrやHetzner等）には、もっと安いプランもある。ただ今回は最初から国内VPSに絞った。円建てで課金される・管理画面とサポートが日本語・国内データセンターなので応答が速い・トラブル時に日本語で問い合わせられる、という安心感を、非エンジニアが最初の1台を持つときの優先事項に置いたからだ。慣れて費用を詰めたくなったら海外VPSへ移ればいい。構成はプロバイダ非依存なので、その移行も難しくない。

申込みからダッシュボードに到達するまでの実画面を順番に貼っておく。

**1. プランを選ぶ**(6GB)

![XServer VPSプラン選択画面（6GBプラン）](/images/hermes-vps/hermes-vps-01-plan-6gb.png)

**2. 申込フォームに必要事項を入力**

![XServer VPS申込フォーム](/images/hermes-vps/hermes-vps-01-application-form.png)
...
![XServer VPSダッシュボード稼働中](/images/hermes-vps/hermes-vps-01-dashboard-running.png)

OSはUbuntu 26.04 LTSを選んだ（XServerは2026年4月に26.04対応済み）。「Dockerアプリイメージ」のような構築済みテンプレートは選ばず、素のUbuntuにする。Dockerは後の回で手動で入れる方針だ。

なぜUbuntu 26.04 LTSを選んだかは次の節で書く。

## Linuxでつまずかないための最低限の知識

「VPSを借りた、で、何をすればいいんだ」という人のために、この記事を読み進めるうえで最低限知っておいたほうがいいLinuxの知識をまとめる。すでに分かっている人は読み飛ばしてかまわない。

### ディストリビューションの選び方

Linuxには「ディストリビューション」と呼ばれる派生バージョンがいくつもある。Windowsでいえば「Windows 11 Home / Pro」のような違い、ではなく、もう少し本格的な分岐で、それぞれ思想と用途が違う。

| ディストリ | 性格 | 採用される場面 |
|---|---|---|
| **Ubuntu** | デスクトップ・サーバーともに情報量No.1。LTSは5年サポート | 個人サーバー、AI開発、初学者 |
| Debian | 安定重視。Ubuntuのベースになっている派生元 | 長期運用前提のサーバー |
| CentOS Stream | Red Hat系。本番用途には推奨されない（無印CentOSは2021年末EOL） | 学習用途のみ |
| Fedora | Red Hat系の最先端。更新サイクルが速い | 開発機・実験用 |

私が **Ubuntu 26.04 LTS** を選んだ理由は3つある。

1. **情報量**：何かに詰まったとき、日本語・英語ともに解決情報が一番多い。NVIDIAドライバやCUDA、Dockerの公式対応もまずUbuntuから始まる
2. **長期サポート**(LTS):26.04 LTSは2031年4月までサポートされる。常時起動のサーバー機には「大型更新に振り回されない安心」が要る
3. **VPSと自宅機の統一**：後に出てくる自宅機（P2）にも同じUbuntu 26.04を入れる予定。2種類のLinuxを覚えずに済む

「とりあえずサーバーを建てたい」目的なら、Ubuntu LTSを選んで間違いはほぼない。
...
シリアルコンソールは、XServerの管理画面からブラウザで直接サーバーにログインできる緊急用の入口だ。まだSSHを設定していない今は、ここが唯一の入口になる。後でSSHの設定をミスして入れなくなったときも、この入口から入って直せるので、最後まで頼りになる命綱だ。
...
![apt update実行中・アップグレード対象3パッケージ表示](/images/hermes-vps/hermes-vps-01-apt-upgrade-progress.png)

私のVPSではアップグレード対象が3パッケージ（`base-files / distro-info-data / motd-news-config`）、ダウンロードサイズ86.4 kBと表示された。

完了すると、カーネル更新が含まれていた場合は「Pending kernel upgrade!」と再起動推奨のメッセージが出る。

![apt upgrade完了後・カーネル更新による再起動推奨表示](/images/hermes-vps/hermes-vps-01-apt-upgrade-complete.png)

カーネルが更新された場合は `reboot` で再起動しておく。再起動後はシリアルコンソールから入り直す。

### 2. 非rootユーザーを作る

rootのまま運用するのは危ない。誤ったコマンドが即・全システム破壊につながる。普段使うユーザーとして `admin` を作り、必要なときだけ `sudo` で管理者権限を借りる、というのがLinuxの作法だ。
...
ここで使うPowerShellは、スタートメニューで「PowerShell」と検索すると出てくる。クリックして開くと黒い画面が立ち上がる。Windows 10/11なら`ssh-keygen`は標準で入っているので、追加インストールは不要だ。開いたら以下を実行する。なお、コマンド中の`~`は「今ログインしているユーザーのフォルダ」を指す記号で、Windowsでは`C:\Users\ユーザー名`に当たる。
...
次のコマンドの`<グローバルIP>`には、自分のサーバーのIPアドレスを入れる。XServerパネルの「VPS管理」ページを開くと、IPアドレス欄にある数字(例:`192.0.2.10`のような4つの数字)がそれだ。以降コマンド中に出てくる`<グローバルIP>`は、すべてここの数字に置き換える。
...
adminでログインできることを確認したら、その流れでrootのSSHログインを締めておく。`root` という名前は世界中の攻撃者に知られていて、22番ポートが開いている限りブルートフォース対象になる。adminだけログインできる状態にしておけば、攻撃面を一段下げられる。

:::message
ここで初めて`nano`というテキストエディタを使う。マウスは効かないので、最初は戸惑う。使い方は次の5つだけ覚えれば足りる。

- カーソル移動は矢印キー
- 文字はそのまま打てば、カーソル位置に挿入される
- 保存は`Ctrl+O`を押してENTER
- 終了は`Ctrl+X`
- 文字を探すときは`Ctrl+W`で検索

画面の下に出る`^O`や`^X`の`^`は「Ctrlキー」の意味だ。`^O`なら「Ctrlを押しながらO」を表している。
:::
...
`PubkeyAuthentication yes`は、公開鍵認証を有効化する設定。デフォルトで`yes`だが、行頭に`#`が付いてコメント扱い(無効化)になっている場合は、その`#`を削除して有効にする(後述のcloud-init上書き対策にもなる)。
...
Ubuntuのsshd_configは、メインファイル(`/etc/ssh/sshd_config`)の先頭で`Include /etc/ssh/sshd_config.d/*.conf`という1行を読み込んでいる。これは「`.d`フォルダの中の設定ファイルもまとめて取り込め」という指示で、クラウド系イメージ(XServerやAWS等)ではcloud-initが `.d` 配下に追加設定を書き込んでくる。
...
OSイメージにUbuntu 26.04を選び、いざブラウザのシリアルコンソールからrootでログインしようとした。だが何度やっても弾かれる。
...
慌てる必要はない。ブラウザのシリアルコンソールは、ポート開放の設定に関係なく入れる。だから「まずコンソールから入り、ネットワークを締める前に必ずコンソールで入れることを確認しておく」のが鉄則になる。SSHが不通になってもコンソールという生還経路が残っていれば、ロックアウトしない。
