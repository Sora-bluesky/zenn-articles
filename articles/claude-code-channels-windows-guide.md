...
:::message
**検証環境**: Windows 11 / Claude Code v2.1.80 / Bun 1.3.11（2026年3月検証）
:::
...
Claude Code がファイルの書き込みやコマンドの実行をする時、通常はターミナルで「許可しますか？」と聞いてくる。チャットアプリからはこの許可を直接与えられないため、**セッションが止まってしまう**。

### おすすめ：Auto-accept モードを設定する

ファイル編集を自動で許可するモードを有効にしておくのが一番手軽。`~/.claude/settings.json` に以下を追加する：
...
これで、ファイルの読み書きは確認なしで実行される。コマンド実行（`npm run build` 等）は引き続き確認が入るので、安全性とのバランスが良い。

:::details その他の対策
**Permission Relay（v2.1.81 以降・公式プラグインは未対応）**

Claude Code v2.1.81 から、許可プロンプトをチャットアプリ側に転送する Permission Relay プロトコルが追加された。ただし、2026年3月時点では公式プラグイン（Discord / Telegram）がこの capability を実装していないため、まだ利用できない。将来対応されれば、スマホから許可・拒否を操作できるようになる。
