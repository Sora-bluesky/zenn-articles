...
- ニュースやX検索を使うジョブを作るなら、第5回でGrok(xai-oauth)を登録済み(X検索=x_searchとWeb検索の本格設定は第9回)
...
![evening-tomorrowジョブの配信例。明日の天気と、その日に応じたごみ収集日がTelegramに届いた画面(市区町村名はマスク)](/images/hermes-vps/hermes-vps-07-cron-evening-result.png)
...
![翌朝7時台、morning-newsジョブが自動で実行され、ニュース要約がTelegramに届いた画面。左上の時刻が朝7時を指しているのがスケジュール実行の証拠](/images/hermes-vps/hermes-vps-07-cron-morning.png)
...
| 検索を使うジョブが失敗する | 第5回でGrok(xai-oauth)を登録済みか、x_searchとWeb検索の設定(第9回)が済んでいるかを確認 |
