...
```bash
hermes cron add '0 17 * * *' '明日の東京の天気(降水確率、最高・最低気温)を一行で。あわせて、明日が燃えるゴミの日(月木)かペットボトルの日(水曜)かも確認して伝える。最後に「明日の予定の確認はいいですか?」と添える。' --name 'evening-tomorrow' --deliver telegram
```
