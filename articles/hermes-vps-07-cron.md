...
手を動かすのは、Telegramのチャット欄に`/cron`コマンドを数行打つだけ。VPS側のファイルを直接いじる必要はない。
...
| ジョブ管理 | 機能なし | `/cron list`等で一覧・停止・削除 |
...
Cronジョブは、第6回で常駐させたHermes Agentの中に登録される。VPSのファイルを手で編集するわけではなく、Telegramから`/cron`コマンドで登録・管理する。
...
第6回までが完了していれば、追加で入れるものはない。Telegramから操作するので、SSHは状態確認だけに使う。
...
Telegramでbotに、次の形のコマンドを1行で送る。スケジュールとプロンプトを引用符でくくり、名前と送り先をオプションで足す。

```text
/cron add '0 7 * * *' '今日のニュースとX上のAI関連の話題を要約して。要約は3〜5項目、各2行以内で。出典URLを必ず付ける。' --name 'morning-news' --deliver origin
```
...
| `--deliver origin` | 結果の送り先(後述) |

:::message
公式の最小サンプルは`--script`でスクリプトの出力をプロンプトに渡す形だが、本記事ではスクリプトを使わず、プロンプトだけのジョブから始める。
:::

<!-- TBD:撮影後に有効化 → ![Telegramで/cron addの1行コマンドを送信した画面(コマンド全体が見える)](/images/hermes-vps/hermes-vps-07-cron-add.png) -->
<!-- TBD:撮影後に有効化 → ![登録完了メッセージ。job_idと名前morning-newsが表示される](/images/hermes-vps/hermes-vps-07-cron-registered.png) -->

### 送り先(delivery target)を決める

`--deliver`で結果をどこへ送るかを指定する。

| 値 | 送り先 | 向いている場面 |
|---|---|---|
| `origin` | ジョブを登録したチャット(デフォルト) | 個人利用なら基本これ |
| `local` | ファイルに保存するだけ、通知なし | ログとして溜める |
| `telegram` | Telegramのホームチャンネルへ送信 | 通知が欲しい時 |
| `discord` / `slack` | 各プラットフォームへ送信 | 複数経路で運用する時 |

個人で使うなら`origin`で十分だ。登録したTelegramのチャットにそのまま返ってくる。

### すぐにテストする

明日の朝7時を待たなくても、今すぐ動作確認できる。まず`/cron list`で登録したジョブのIDを確認し、そのIDを指定して即時実行する。

```text
/cron run <job_id>
```

数秒から数十秒でTelegramに要約が届けば成功だ。出典URLが付いているか、3〜5項目で出ているかを確認する。

<!-- TBD:撮影後に有効化 → ![/cron run <job_id>送信→「実行開始」相当の即時応答](/images/hermes-vps/hermes-vps-07-cron-run.png) -->
<!-- TBD:撮影後に有効化 → ![数秒後、Telegramにニュース要約が3〜5項目+出典URL付きで届いた画面](/images/hermes-vps/hermes-vps-07-cron-result.png) -->

## ジョブを管理する

登録したジョブは、すべてTelegramの`/cron`コマンドで管理する。実行・停止・削除のときは、`/cron list`で表示される**job_id**で対象を指定する(`--name`で付けた名前は一覧で見分けるためのラベルで、操作はIDで行う)。

| コマンド | 役割 |
|---|---|
| `/cron list` | 登録済みジョブの一覧(job_idと名前が出る) |
| `/cron run <job_id>` | 即時実行(テスト用) |
| `/cron pause <job_id>` | 一時停止(削除はしない) |
| `/cron edit <job_id>` | スケジュールやプロンプトを変更 |
| `/cron remove <job_id>` | 完全に削除 |

<!-- TBD:撮影後に有効化 → ![/cron listの結果。morning-newsが1件、active状態でjob_idと名前が見える](/images/hermes-vps/hermes-vps-07-cron-list.png) -->
<!-- TBD:撮影後に有効化 → ![/cron pause <job_id>を実行してpausedと表示された画面](/images/hermes-vps/hermes-vps-07-cron-pause.png) -->
<!-- TBD:撮影後に有効化 → ![/cron listでmorning-newsがpaused状態になった画面](/images/hermes-vps/hermes-vps-07-cron-list-paused.png) -->

:::message
公式ドキュメントに載っているサブコマンドは`add` / `list` / `run` / `pause` / `edit` / `remove`の6つ。一時停止したジョブを再び動かす専用の「resume」コマンドは用意されていない。長く止めるなら`remove`で消して必要なときに登録し直すほうが分かりやすい。
:::
...
夕方17時に「明日の天気とゴミ出し予定」を送る例。

```text
/cron add '0 17 * * *' '明日の東京の天気(降水確率、最高・最低気温)を一行で。あわせて、明日が燃えるゴミの日(月木)かペットボトルの日(水曜)かも確認して伝える。最後に「明日の予定の確認はいいですか?」と添える。' --name 'evening-tomorrow' --deliver origin
```

毎週金曜23時に「今週の振り返り」を促す例。

```text
/cron add '0 23 * * 5' '今週の振り返りを促す質問を3つ送って。仕事・個人・学習の各カテゴリから1つずつ。「答えてくれたら、来週の優先順位の整理を手伝う」と添える。' --name 'friday-reflection' --deliver origin
```

登録したら`/cron list`で2件並んでいることを確認し、`/cron run <job_id>`で動作を見ておく。

<!-- TBD:撮影後に有効化 → ![2つ目のジョブの登録完了画面(job_idと名前が表示される)](/images/hermes-vps/hermes-vps-07-cron-add2.png) -->
<!-- TBD:撮影後に有効化 → ![/cron listで2件のジョブが表示された画面](/images/hermes-vps/hermes-vps-07-cron-list2.png) -->
<!-- TBD:撮影後に有効化 → ![2つ目のジョブを/cron run <job_id>で即時実行し、結果がTelegramに届いた画面](/images/hermes-vps/hermes-vps-07-cron-run2.png) -->

## まとめと第8回予告

第7回でやったこと。

- `/cron add`の1行コマンドで、毎朝7時のニュース要約ジョブを登録
- `--deliver origin`で結果を登録元のTelegramチャットへ
- `/cron run`で即時テスト、`/cron list` / `pause` / `edit` / `remove`で管理
...
| 症状 | 対処 |
|---|---|
| Cronが指定時刻に動かない | 1) `systemctl --user is-active hermes-gateway`で常駐を確認、2) `/cron list`でジョブが`paused`になっていないか、3) cron式が5フィールドになっているか |
| 結果が届かない | プロンプトに「変化がなければ`[SILENT]`」と書いていて、エージェントが`[SILENT]`を返した可能性。`/cron edit`でその指示を外して再実行 |
| 「過去の話の続き」を求める返事が来る | self-contained promptになっていない。`/cron edit`でプロンプトを書き直す(依頼書として全部書き切る) |
| Telegramに二重で届く | 同じジョブが重複登録されている可能性。`/cron list`で確認し`/cron remove`で片方を削除 |
