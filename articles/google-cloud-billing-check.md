...
- **Gemini API のテキスト生成は課金不要。** 無料枠で Gemini 2.5 Pro / Flash が使える
- **不要な課金は 3 ステップで止められる。** プロジェクトの課金無効化 / 請求先アカウント閉鎖 / プロジェクト削除
...
本記事は **確認 → 判断 → 解約** の 3 ステップで構成されている。

**Step 1. 確認する**
- [方法 1 - Google Cloud Console で確認する](#方法-1---google-cloud-console-で確認する)
- [方法別の使い分けまとめ](#方法別の使い分けまとめ)

**Step 2. 判断する**
- [よくある課金の勘違い](#よくある課金の勘違い)
- [課金が必要なケースと不要なケース](#課金が必要なケースと不要なケース)
- [Gemini API の課金判断フローチャート](#gemini-api-の課金判断フローチャート)

**Step 3. 解約する**
- [方法 A - 特定プロジェクトの課金を無効にする](#方法-a---特定プロジェクトの課金を無効にする)
...
:::message alert
インストール後、必ずターミナルを開き直してほしい。既に開いているターミナルでは PATH（パソコンがコマンドを探す場所の一覧）が反映されず、`gcloud` コマンドが見つからないエラーになる。
:::

#### 方法 B - winget を使う

winget は Windows 標準のアプリインストールコマンドだ。
...
| サービス | 無料枠 |
|---|---|
| **Gemini API（テキスト生成）** | 2.5 Pro: 5 RPM（1分あたり5回）・25 RPD（1日あたり25回） / Flash: 15 RPM・500 RPD |
...
| | 予算アラート | 異常検知アラート |
|---|---|---|
| **仕組み** | 自分で設定した金額を超えたら通知 | Google が過去の利用パターンから異常を自動検知 |
| **事前設定** | 必要（金額を指定） | 不要（自動） |
| **おすすめの使い方** | 「月5ドルを超えたら知らせて」 | 「普段と違う使い方をしたら知らせて」 |
