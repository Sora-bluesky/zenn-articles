...
AI コーディングの文脈では、Claude Code で作ったものを公開するのも GitHub 経由が最も簡単である。
...
:::message
### `origin` って何？
`origin` は GitHub 上のリポジトリに付けた **名前**（ニックネーム）。`github` でも `myremote` でも何でもよいが、 **慣例として `origin` を使う**。引っ越ししたら住所（URL）を変えるだけで、名前はそのまま使える。
:::
...
:::message
### fetch と pull の違い（知っておくと安心）
| 操作 | やること | ファイルへの影響 |
|------|---------|---------------|
| `fetch` | GitHub の最新情報を「見るだけ」 | なし（安全） |
| `pull` | fetch + merge（取り込みまで行う） | ファイルが更新される |

pull で不安なときは、先に `git fetch` だけして確認してから `git merge` する、という手順も取れる。慣れないうちは `pull` だけ覚えておけば十分。
:::
...
### 「push したら rejected (non-fast-forward) と出た」

GitHub のほうが自分のローカルより新しい場合に発生する。他の人（や別のPCの自分、または AI ツール）が先に push していた。
...
2. コンフリクトがあれば[第 6 章](06-branch-and-merge)の手順で解消する
3. もう一度 push する
...
:::message alert
### 絶対にやってはいけない: `git push --force`
`--force` は他の人の変更を上書きして消す可能性がある、最も危険な Git 操作の一つ。 **この本で紹介する範囲では `--force` が必要になる場面はない。** Web で見つけた解決策に `--force` が含まれていたら、立ち止まって別の方法を探すこと。
:::
...
:::message
push を毎回楽にする設定 → [第 9a 章「push.autoSetupRemote」](09a-git-config-reference)を参照
:::
