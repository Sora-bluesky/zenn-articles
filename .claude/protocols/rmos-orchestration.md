...
#### 4. 実機検証の事前確認（ツール系記事の場合）
```
確認項目:
- [ ] 使用するLLMプロバイダーは？（Anthropic / OpenAI / Google / その他）
- [ ] APIキーは取得済みか？
- [ ] 無料枠の有無と制限は確認済みか？
- [ ] 他のアプリ（AI Studio、Antigravity等）でクォータを消費していないか？
- [ ] 検証環境は？（Windows PowerShell / WSL2 / macOS / Linux）
```
...
:::message alert
**過去の失敗事例**: 複数記事を同時公開しようとしてレートリミットに引っかかり、デプロイされなかった。リトライを繰り返した結果、制限が延長された可能性あり。
:::

**公開前チェック:**
- [ ] 今日すでにZennに記事を公開していないか確認
- [ ] 複数記事を同時公開しようとしていないか確認
- [ ] `git force-push` ではなく通常の `git push` を使用
- [ ] 細かい修正は1回のコミットにまとめてからpush

**レートリミットエラー（HTTP 429）が発生した場合:**
1. リトライを繰り返さない（逆効果の可能性）
2. 24時間程度待ってから再試行
3. 3記事以上の同時公開が必要な場合は事前にZennに申請

**注意:** 具体的な制限数値は公式には非公開。上記はユーザー観測に基づく目安。

**公式根拠:**
- [Zenn利用規約](https://zenn.dev/terms) 第12条（サーバーに負担をかける行為の禁止）
- [2025年6月ガイドライン改定](https://info.zenn.dev/2025-06-02-guideline-update)
...
## Phase 5: Hands-on Verification（実機検証）

**実行者**: メインエージェント（ユーザーの環境で検証）

**Goal**: 記事の手順通りに実際に動かし、ハマりポイントを発見・記録する。
...
### Input
- `article_draft`（Phase 3で作成した記事）
- 検証環境

### Process
1. **クリーンな環境で記事の手順を実行**
2. **全てのコマンドを実際に実行し、出力を記録**
3. **エラーや警告があれば原因を特定**
4. **記事に記載のない前提条件や注意点を発見**
5. **トラブルシューティング手順を検証**

### 検証チェックリスト
- [ ] インストールコマンドは正常に完了するか
- [ ] 設定コマンドは正常に反映されるか
- [ ] 動作確認の手順で期待通りの結果が得られるか
- [ ] エラーが発生した場合、記事のトラブルシューティングで解決できるか
- [ ] 環境依存の問題（WSL2必須など）は明記されているか
- [ ] APIクォータ・料金に関する注意が記載されているか

### よくある発見事項（過去の経験から）

| 発見 | 対処 |
|------|------|
| PowerShellから実行できない | WSL2必須の警告を冒頭に追加 |
| 設定変更が反映されない | セッションクリア手順を追加 |
| 429エラー（クォータ超過） | 代替モデルと待機時間を記載 |
| 他アプリとクォータ共有 | 警告セクションを追加 |

### Output
- `verification_log`: 検証ログ
- `discovered_issues`: 発見した問題リスト
- `article_updates`: 記事への追記・修正リスト

### 分岐
- 問題なし → Phase 4（公開準備）へ
- 軽微な問題 → 記事を修正して Phase 4 へ
- 重大な問題 → Phase 3 へ戻り記事を大幅修正
